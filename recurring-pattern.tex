...
As the name suggests, headless relatives are relative clauses that lack an (overt) head. The internal case, the case from the relative clause, and the external case, the case from the main clause, compete to surface on the relative pronoun. In this section I first discuss examples in which the internal and the external match, and then examples in which they differ.

If the internal and the external case are identical, the relative pronoun simply surfaces in that case. I illustrate this with examples from headless relatives in Gothic. I give examples in which the internal and external case are respectively nominative, accusative and dative. The relative pronoun appears in the respective cases.
...
The internal case is nominative, as the predicate \tit{matjan} `to eat' takes nominative subjects.
The external case is nominative as well, as the predicate \tit{ga-dauþnan} `to die' takes nominative subjects.
The relative pronoun \tit{sa} `\ac{rel}.\ac{sg}.\ac{m}.\ac{nom}' appears in the internal and external case: the nominative.
...
Consider the example in \ref{ex:gothic-acc-acc-rep}, in which the internal accusative case competes against the external accusative case.
The internal case is accusative, as the predicate \tit{arman} `to pity' takes accusative objects.
The external case is accusative as well, the predicate \tit{ga-arman} `to pity' takes accusative objects.
The relative pronoun \tit{þan(a)} `\ac{rel}.\ac{sg}.\ac{m}.\ac{acc}' appears in the internal and external case: the accusative.
...
Consider the examples in \ref{ex:gothic-dat-dat}, in which the internal dative case competes against the external dative case.
The internal case is dative, as the predicate \tit{manwjan} `to prepare' takes dative indirect objects.
The external case is dative as well, the predicate \tit{giban} `to give' takes dative indirect objects.
The relative pronoun \tit{þaim)} `\ac{rel}.\ac{sg}.\ac{m}.\ac{dat}' appears in the internal and external case: the dative.
...
These findings can be summarized as in Table \ref{tbl:summary-gothic-matching}. The left column shows the internal case (\ac{int}) between square brackets. The upper row shows the external case (\ac{ext}) between square brackets. The other cells indicate the case of the relative pronoun.

The diagonal from the top left to the bottom right corresponds to the examples given so far in which the internal and external case match. The nominative marked in light gray corresponds to \ref{ex:gothic-nom-nom}, in which the internal nominative case competes with the external nominative case, and the relative pronoun surfaces in the nominative case. The accusative marked in dark gray corresponds to \ref{ex:gothic-acc-acc}, in which the internal accusative case competes with the external accusative case, and the relative pronoun surfaces in the accusative case. The unmarked dative corresponds to \ref{ex:gothic-dat-dat}, in which the internal dative case competes with the external dative case, and the relative pronoun surfaces in the dative case.

\begin{table}[H]
 \center
 \caption{Extensive summary of Gothic headless relatives (matching)}
...
The internal case is accusative, as the predicate \tit{frijon} `to love' takes accusative objects.
The external case is nominative, as the predicate \tit{wisan} `to be' takes nominative subjects.
The relative pronoun \tit{þan(a)} `\ac{rel}.\ac{sg}.\ac{m}.\ac{acc}' appears in the internal case: the accusative. The relative pronoun is marked in bold, just like as the relative clause, showing that the relative pronoun patterns with the relative clause.
...
The internal case is nominative, as the predicate \tit{wisan} `to be' takes nominative subjects.
The external case is accusative, as the predicate \tit{ussiggwan} `to read' takes accusative objects.
The relative pronoun \tit{þo} `\ac{rel}.\ac{sg}.\ac{n}.\ac{acc}' appears in the external case: the accusative. The relative pronoun is not marked in bold, just like as the main clause, showing that the relative pronoun patterns with the main clause.
...
\begin{table}[H]
 \center
 \caption{Extensive summary of Gothic headless relatives (\ac{nom} --- \ac{acc})}
 \begin{tabular}{c|c|c|c}
   \toprule
     \textsubscript{\ac{int}} \textsuperscript{\ac{ext}}
         & [\ac{nom}]
...
The internal case is dative, as the predicate \tit{fraletan} `to forgive' takes dative objects.
The external case is nominative, as the predicate \tit{frijon} `to love' takes nominative subjects.
The relative pronoun \tit{þamm(a)} `\ac{rel}.\ac{sg}.\ac{m}.\ac{dat}' appears in the internal case: the dative. The relative pronoun is marked in bold, just as the relative clause, showing that the relative pronoun patterns with the relative clause.
...
The internal case is nominative, as the predicate \tit{wisan} `to be' takes nominative subjects.
The external case is dative, as the predicate \tit{fraþjan} `to think about' takes dative indirect objects.
The relative pronoun \tit{þaim} `\ac{rel}.\ac{pl}.\ac{n}.\ac{dat}' appears in the external case: the dative. The relative pronoun is not marked in bold, just like as the main clause, showing that the relative pronoun patterns with the main clause.
...
The external case is accusative, as the predicate \tit{ushafjan} `to pick up' takes accusative objects.
The relative pronoun \tit{þamm(a)} `\ac{rel}.\ac{sg}.\ac{n}.\ac{dat}' appears in the internal case: the dative. The relative pronoun is marked in bold, just like as the relative clause, showing that the relative pronoun patterns with the relative clause.
...
The internal case is accusative, as the predicate \tit{insandjan} `to send' takes accusative objects.
The external case is dative, as the predicate \tit{galaubjan} `to believe' takes dative objects.
The relative pronoun \tit{þamm(a)} `\ac{rel}.\ac{sg}.\ac{m}.\ac{dat}' appears in the external case: the dative. The relative pronoun is not marked in bold, just like as the main clause, showing that the relative pronoun patterns with the main clause.
...
Table \ref{tbl:summary-gothic-acc-dat} can be simplified as Figure \ref{tbl:summary-gothic-simple}. The data can be divided into three set: (1) a set of three unmarked cells in the diagonal from the top left to the bottom right, (2) a set of three light gray marked cells in the lower left corner and (3) a set of three dark gray marked cells in the upper right corner.

The unmarked three cells in the diagonal are situations in which the internal and the external case match. The three cells in the lower left corner, marked in light gray, are the situations in which the internal case surfaces when it wins the competition. The three cells in the upper right corner, marked in dark gray, are the situations in which the external case surfaces when it wins the competition.

In the three instances in the lower left corner, the relative pronoun appears in the internal case. They correspond to the examples \ref{ex:gothic-nom-acc-rep}, \ref{ex:gothic-nom-dat} and \ref{ex:gothic-acc-dat}.
In the three instances in the upper right corner, the relative pronoun appears in the external case. They correspond to the examples in \ref{ex:gothic-acc-nom-rep}, \ref{ex:gothic-dat-nom} and \ref{ex:gothic-dat-acc}.
