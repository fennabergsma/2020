...
\begin{figure}[ht]
  ...
  \end{tikzpicture}
  \caption{Agreement hierarchy (grammatical function) \citep{moravcsik1978}}
...
Putting the languages in \posscitet{moravcsik1978} figure gives the following result.
 ...
 \begin{figure}[ht]
   ...
   \end{tikzpicture}
   \caption{Agreement hierarchy with languages \citep{moravcsik1978}}
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}
    ...
    \node[] at (1.375,0) {\footnotesize{● Basque}};
  \end{tikzpicture}
  \caption{Agreement hierarchy (case) \citep{bobaljik2006}}
...
\begin{figure}[ht]
  ...
  \begin{tikzpicture}
    ...
    \node[] at (1.375,0) {\footnotesize{● Basque}};
  \end{tikzpicture}
  \caption{Relativization hierarchy (grammatical function)}
...
\citet{keenan1977} point out that some ergative-absolutive languages form a counterexample to their hierarchy. One example comes from Dyirbal. Absolutive objects and absolutive subjects can be relativized. Ergative subjects, however, cannot.

\ex.\label{ex:dyirbal-decl}
\ag. balan d̦ugumbil ɲina-ɲu\\
 \tsc{det}.\tsc{abs} woman.\tsc{abs} sit-\tsc{pass}\\
 `The woman is sitting down.'\label{ex:dyirbal-sub}
\bg. ŋad̦a balan d̦ugumbil buŗa-n\\
 I.\tsc{erg} \tsc{det}.\tsc{abs} woman.\tsc{abs} see-\tsc{pres/past}\\
 `I am watching the woman.'\flushfill{Dyirbal, \pgcitealt{dixon1972}{100}, my boldfacing}\label{ex:dyirbal-do}

The embedded verb takes the marker -\tbf{ŋu} to indicate that there is a relative clause.

\exg. balan \tbf{d̦ugumbil} ŋad̦a buŗa-\tbf{ŋu} ɲina-ɲu\\
 \tsc{det}.\tsc{abs} woman.\tsc{abs} I see-\tsc{rel} sit-\tsc{pass}\\
 `The woman whom I am watching is sitting down.'\flushfill{Dyirbal, \pgcitealt{dixon1972}{100}, my boldfacing}\label{ex:dyirbal-abs-sub}

\exg. ŋad̦a balan \tbf{d̦ugumbil} ɲina-\tbf{ŋu} buŗa-n\\
 I.\tsc{erg} \tsc{det}.\tsc{abs} woman.\tsc{abs} sit-\tsc{rel} see-\tsc{pres/past}\\
 `I am watching the woman who is sitting down.'\flushfill{Dyirbal, \pgcitealt{dixon1972}{100}, my boldfacing}\label{ex:dyirbal-abs-do}

Ergatives need to first be promoted to absolutives, creating a passive-like structure, and then the absolutive can be relativized.

johnson abs/erg (keenan/comrie) already claimed the abs/erg absolutive relativization should be captured in terms of case and not grammatial function.
...
Second, Finnish is a language that has a particular relativization strategy for unmarked and dependent case, as shown in X.
