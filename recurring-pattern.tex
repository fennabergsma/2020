...
Consider the example in \ref{ex:gothic-nom-nom}, in which the internal case and the external case are nominative.
The relative clause, including the relative pronoun, is marked in gray.
The internal case is nominative. The predicate \tit{matjan} `to eat' takes nominative subjects.
The external case is nominative as well. Here the predicate \tit{ ga-dauþnan} `to die' takes nominative subjects.
...
Consider the example in \ref{ex:gothic-acc-acc-rep}, repeated from the introduction. In this example, the internal case and the external case are accusative.
The relative clause, including the relative pronoun, is marked in gray.
The internal case is accusative. The predicate \tit{arman} `to pity' takes accusative objects.
The external case is accusative as well. Here the predicate \tit{ga-arman} `to pity' takes accusative objects.
The relative pronoun \tit{þan(a)} `who.\ac{acc}' appears in the accusative.

The internal case is accusative. The predicate \tit{bleiþjan} `to have mercy on' takes accusative objects.
The external case is accusative as well. Here the predicate \tit{ga-bleiþjan} `to have mercy on' takes accusative objects.

\ex.\label{ex:gothicaccaccrep}
\ag. gaarma \tbf{þan} \tbf{-ei} \tbf{arma}\\
 pity.1\ac{sg}\scsub{[acc]} \ac{rel}.\ac{sg}.\ac{m}.\ac{acc} -\ac{comp} pity.1\ac{sg}\scsub{[acc]}\\
 `I pity him, whom I pity'
\bg. jah gableiþja \tbf{þan} \tbf{-ei} \tbf{bleiþja}\\
 {have mercy}.1\ac{sg}\scsub{[acc]} \ac{rel}.\ac{sg}.\ac{m}.\ac{acc} -\ac{comp} {have mercy}.1\ac{sg}\scsub{[acc]}\\
 `I have mercy on him, whom I have mercy on' \flushfill{Gothic, \ac{rom} 9:15, after \pgcitealt{harbert1978}{339}}

Consider the examples in \ref{ex:gothic-dat-dat}, in which the internal case and the external case are dative.
...
\ag. \tbf{þamm} \tbf{-ei} \tbf{gabaur} gabaur\\
 \ac{rel}.\ac{sg}.\ac{m}.\ac{dat} -\ac{comp} tribute\scsub{[dat]} tribute\scsub{[dat]}\\
 `tribute to (him) whom tribute is due'
\bg. \tbf{þamm} \tbf{-ei} \tbf{mota} mota\\
 \tbf{who.\ac{dat}} \tbf{-\ac{comp}} \tbf{custom\scsub{[dat]}} custom\scsub{[dat]}\\
 `custom to (him) whom custom is due'
\bg. \tbf{þamm} \tbf{-ei} \tbf{agis} agis\\
 \tbf{who.\ac{dat}} \tbf{-\ac{comp}} \tbf{fear\scsub{[dat]}} fear\scsub{[dat]}\\
 `fear (him) whom fear is due'
\bg. \tbf{þamm} \tbf{-ei} \tbf{sweriþa} sweriþa\\
 \tbf{who.\ac{dat}} \tbf{-\ac{comp}} \tbf{honour\scsub{[dat]}} honour\scsub{[dat]}\\
...
From here the competition starts.
