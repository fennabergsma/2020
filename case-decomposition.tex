...
Table \ref{tbl:dat-against-acc} shows the situation in which the dative and the accusative compete against each other. The bigger structure deletes the smaller structure: the dative wins.
\footnote{
...
Next is the competition between dative and nominative, in which dative wins. The corresponding cells are marked light gray in Table \ref{tbl:summary-gothic-deriving}. In \ref{ex:dat-contains-nom} I show the syntactic structure of a dative relative pronoun. It contains the syntactic features for a relative pronoun, \tsc{f1}, \tsc{f2} and \tsc{f3}. Contained in this structure is an nominative relative pronoun, marked in gray. This consists of the syntactic features for the relative pronoun and \tsc{f1}.
...
\begin{table}[H]
  \center
	\caption {\ac{dat}P deletes \ac{nom}P}
      ...
      \end{forest} \\
  \end{tabular}
\end{table}\label{ex:dat-against-nom}

Finally there is the competition between accusative and nominative, in which accusative wins. The corresponding cells are uncolored in Table \ref{tbl:summary-gothic-deriving}. In \ref{ex:acc-contains-nom} I show the syntactic structure of an accusative relative pronoun. It contains the syntactic features for a relative pronoun, \tsc{f1} and \tsc{f2}. Contained in this structure is a nominative relative pronoun, marked in gray. This consists of the syntactic features for the relative pronouna and \tsc{f1}.
...
In this section I discussed how the case scale in case competition of headless relative can be derived from a cumulative case decomposition. The idea is that a case wins the competition if it contains all features the other case has. As the dative is featurally the richest (it contains \tsc{f1}, \tsc{f2} and \tsc{3}), it wins over the accusative (which consists of \tsc{f1} and \tsc{f2}) and the nominative (which contains only \tsc{f1}). Finally, the accusative wins over the nominative, because the former is featurally richer than the latter.
