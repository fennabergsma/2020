...
What I showed as well in the previous chapter is that the case scale \ac{nom} < \ac{acc} < \ac{dat} is a wide-spread phenomenon: the recurs in different phenomena across several languages. The scale can be observed in at least two more syntactic phenomena: agreement and relativization.\footnote{
...
The case scale also appears within morphology in syncretism patterns and morphological case containment. \pgcitet{pittner1995}{201:fn.4} makes this link to morphology as well: ``Furthermore, the Case hierarchies receive some independent support by morphology as shown by the various inflectional paradigms.''
...
In Table \ref{tbl:syncretisms-derive} I repeat from Table \ref{tbl:syncretisms} in Section \ref{sec:syncretism} examples that shows the possible and impossible syncretism patterns.
...
The lexical entry for \tit{tú} is given in \ref{ex:faroese-tu-lexicon}. The lexical tree consists of the second person singular pronoun (the 2\ac{sg}P), and \ac{f}1, making it a \ac{nom}P. The phonological representation that is linked to the lexical tree is \tit{tú}.\footnote{
...
The lexical entry for \tit{teg} is given in \ref{ex:faroese-teg-lexicon}. The lexical tree consists of all the features of the lexical tree in \ref{ex:faroese-tu-lexicon}, plus \ac{f}2, making it an \ac{acc}P. The linked phonological representation is \tit{teg}.
...
The lexical entry for \tit{tær} is given in \ref{ex:faroese-taer-lexicon}. The lexical tree consists of all the features of the lexical tree in \ref{ex:faroese-teg-lexicon}, plus \ac{f}3, making it an \ac{dat}P. The linked phonological representation is \tit{tær}.
...
In the Faroese examples above, the syntactic structure are all identical to the lexical trees. However, Nanosyntax assumes that to be a successful match, identity is not a necessary requirement. Instead, matching relies on a containment relation. A lexical entry applies when it contains all features. This is formalized as in \ref{ex:superset-principle}.
...
Consider the Icelandic pattern. For the first person plural, Icelandic uses \tit{við} as nominative and \tit{okkur} as accusative and dative. Two lexical entries are needed for that. The first one in \ref{ex:icelandic-vid-lexicon} contains pronominal features and \ac{f}1, and corresponds to the phonology \tit{við}.
The second one is given in \ref{ex:icelandic-okkur-lexicon}. It contains in addition to \ref{ex:icelandic-vid-lexicon} also the feature \ac{f}2 and \ac{f}3. The phonological representation that is linked to it is \tit{okkur}.
...
The first one in \ref{ex:german-sie-lexicon} contains pronominal features, \ac{f}1 and \ac{f}2. It corresponds to the phonology \tit{sie}.
The second one is given in \ref{ex:german-ihr-lexicon}. It contains in addition to \tit{sie} in \ref{ex:german-sie-lexicon} also the feature \ac{f}3. It corresponds to the phonology \tit{ihr}.
...
Before I show how languages with morphological case containment can be derived, I need to discuss how variation between languages is modeled in Nanosyntax. Crosslinguistic variation is namely explained purely in terms of differences in the lexicon. In other words, the syntactic structure is identical across languages, but the lexical entries package features together differently.

Making this more concrete, the dative contains \ac{f}1, \ac{f}2 and \ac{f}3 in all languages. In the languages discussed in the previous section, Section \ref{sec:syncretism}, \ac{f}3 can only be spelled out in a single lexical entry together with \ac{f}1 and \ac{f}2. The result is that the examples are syncretic (i.e. formally identical) or suppletive (i.e. formally unrelated). I show in this section that this is different for languages that show morphological case containment. The \ac{f}2 and the \ac{f}3 are both spelled out by the own respective lexical entries. The result is that the accusative formally contains the nominative, and the dative formally contains the accusative.

Let me illustrate this by deriving the 3\ac{sg} paradigm in Khanty.
First, I give the lexical entry for the nominative third person singular. It contains pronominal features and the feature \ac{f}1. The phonological form associated with the structure is \tit{luw}. The lexical entry is given in \ref{ex:khanty-luw-lexicon}.
...
As shown in \ref{tbl:cont-khanty-3sg}, the morphological form of the accusative contains the morphological form of the nominative plus an extra morpheme: \tit{e:l}. As shown in \ref{tbl:case-decomposed}, the syntactic features of the accusative contain the syntactic features of the nominative plus an extra feature: \ac{f}2. Accordingly, I give the lexical entry for the accusative marker \tit{e:l} in \ref{ex:khanty-el-lexicon}.\footnote{
...
The lexical entry in \ref{ex:khanty-el-lexicon} can only match the syntactic structure if \ac{nom}P moves away, leaving the \ac{acc}P containing \ac{f}2 behind. In other words, the complement of \ac{f}2 needs to be moved away.
...
This type of movement is different from syntactic movement. It is driven by spellout, it does not have any interpretational effects, and it does not leave any traces \citep{starke2018}. In Section \ref{ch:relativization} I return to `regular' syntactic movement in Nanosyntax.
}
...
This is exactly the type of movement I described as necessary for the Khanty pronoun. The movement is displayed in \ref{ex:khanty-luw-el-movement}. The complement of \ac{f}2, the \ac{nom}P, the lower right circled portion in the structure, moves to the specifier of \ac{acc}P.\footnote{
...
The result of the movement is given in \ref{ex:khanty-luw-el-spellout}. The lexical tree in \ref{ex:khanty-el-lexicon} matches the syntactic structure, and \ac{acc}P is spelled out as \tit{e:l}.
