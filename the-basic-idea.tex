...
CONSTITUENT
how to formulate the syncretism thing in terms of constituent containment

For light heads and relative pronouns this means that one of them can be absent when they form a constituent within the other element. Throughout this chapter I elaborate further on the exact requirements for constituent containment.\footnote{
There are namely two types of containment possible. The first one is structural constituent containment: an element can be absent if it structurally forms a constituent within the other element. I elaborate on this in Section \ref{sec:basic-matching}. The second one is formal containment: an element can be absent if its form is syncretic with a constituent within the other element. I elaborate on this in Section \ref{sec:basic-unrestricted}.}
...
\begin{figure}[htbp]
  ...
  \begin{tabular}[b]{ccc}
      ...
      \bottomrule
  \end{tabular}
   \caption {\tsc{ext}\scsub{∅} vs. \tsc{int}\scsub{nom} in the unrestricted type}
  \label{fig:empty-nom-unres}
\end{figure}

then the F1 is merged
