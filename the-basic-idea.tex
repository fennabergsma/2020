...
In what follows I show a situation similar to the missing \tsc{rel} feature: a syncretism between nominative and accusative case in Modern German. This situation leads me to propose a second type of constituent containment: formal constituent containment.
...
The light head corresponds to \tit{P}, illustrated by the circle around the \tsc{k}P and the \tit{P} below it. The relative pronoun corresponds to \tit{P} too, illustrated by the circle around the \tsc{rel}P and the \tit{P}. The relative pronoun (the \tsc{rel}P realized by \tit{P}) is a constituent that is formally contained within the light head (the \tsc{k}P realized by \tit{P}), so the relative pronoun can be deleted.

Although in this situation the relative pronoun can be deleted, this does not describe the situation in Old High German, the language I discuss in Chapter \ref{ch:deriving-unrestricted}. In Old High German, the second possible light head contains an additional feature, which I refer to as \tsc{x}.
...
As discussed, I propose that this light head does not only consist of phi and case features, but that it also contains a feature I here refer to as \tsc{x}. In Chapter \ref{ch:deriving-unrestricted} I motivate this claim and I discuss the content of this feature.
...
The feature \tsc{x} is spelled out by its own lexical entry, indicated by the circle around the \tsc{x}P. The rest of the light head is spelled out by the portmanteau of phi and case features.
The relative pronoun always consists of all features the light head consists of. Therefore, it consists of phi and case features, the feature \tsc{rel} and the feature \tsc{x}.\footnote{
I actually assume that the relative pronoun that is being compared to the first possible light head also contains the feature \tsc{x}. I left it out of the structures there because it was not relevant for the discussion.
}
The phi and case features are spelled out by the same portmanteau as in the light head. The feature \tsc{rel} and the feature \tsc{x} are spelled out by a single lexical entry.
It is crucial for the analysis that there is a lexical entry for the \tsc{rel}P which contains the feature \tsc{rel} and the \tsc{x}P, but not a more specific one that spells out the \tsc{x}P on its own. In \ref{ex:entry-x}, I give the lexical entry, which spells out as \tit{X}.
...
The light head corresponds to \tit{XY}, illustrated by the circle around the \tsc{x}P and the \tit{X} below it and the circle around the \tsc{nom}P and the \tit{Y} below it. The relative pronoun corresponds to \tit{XY} too, illustrated by the circle around the \tsc{rel}P and the \tit{X} below it and the circle around the \tsc{nom}P and the \tit{Y} below it.
I draw a dotted circle around each constituent that is a constituent in both the light head and the relative pronoun.
The relative pronoun (the \tsc{rel}P realized by \tit{XY}) is a constituent that is formally contained within the light head (the \tsc{x}P realized by \tit{XY}), so the relative pronoun is deleted.\footnote{
The same holds the other way around: the light head (the \tsc{x}P realized by \tit{XY}) is a constituent that is formally contained within the relative pronoun (the \tsc{rel}P realized by \tit{XY}). Therefore, either the light head or the relative pronoun can be deleted. I delete the relative pronoun here, as I discuss how it is possible for the relative pronoun to be deleted even though it has a feature less than the light head.
}
...
The light head corresponds to \tit{XZ}, illustrated by the circle around the \tsc{x}P and the \tit{X} below it and the circle around the \tsc{acc}P and the \tit{Z} below it. The relative pronoun corresponds to \tit{XY}, illustrated by the circle around the \tsc{rel}P and the \tit{X} below it and the circle around the \tsc{nom}P and the \tit{Y} below it.
I draw a dotted circle around each constituent that is a constituent in both the light head and the relative pronoun.
...
Focusing on the second possible light head, languages of the unrestricted type have a lexical entry that spells out phi and case features and a lexical entry that spells out the features X and \tsc{rel} and crucially not a lexical entry that only spells out the feature \tsc{x}.
Headless relatives in this language are grammatical in all situations: when the internal and the external case match, when the internal case is more complex and when the external case is more complex.
...
In Figure \ref{fig:rel-lh-unres-mono-rep}, the relative pronoun is a constituent that is formally contained within the light head, and the relative pronoun can be deleted. Note here that the internal and external case need to be identical too. Only then the two forms are fully syncretic, and deletion can take place.
