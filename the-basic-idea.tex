...
The \tsc{acc}P in the light head corresponds to \tit{-as}, illustrated by the circle around the \tsc{acc}P and the \tit{-as} under it. The \tsc{nom}P in the relative pronoun corresponds to \tit{-as} too, illustrated in the same way.
I draw a dotted circle around each constituent that is a constituent in both the light head and the relative pronoun.
The light head (the \tsc{acc}P realized by \tit{-as}) is syncretic with a constituent within the relative pronoun (the \tsc{nom}P realized by \tit{-as}).
As the light head is deleted, the headless relative surfaces with the relative pronoun that bears the internal case.
...
I propose that this light head does not only consist of phi- and case-features, but that they also contain a definite feature \tsc{d}. In Chapter \ref{ch:deriving-unrestricted} I motivate this claim. The light head is spelled out by two lexical entries. The definite feature is spelled out by its own lexical entry, indicated by the circle around the \tsc{d}P. The rest of the light head is spelled out by the portmanteau of phi- and case-features.
...
Therefore, it consists of phi- and case-features, the feature \tsc{rel} and the feature \tsc{def}.
...
It is crucial for the analysis that there is a lexical entry for the \tsc{rel}P which contains the feature \tsc{rel} and the \tsc{d}P, but not a more specific one that spells out the \tsc{d}P on its own. In \ref{ex:entry-d}, I give the lexical entry, which spells out as \tit{d-}.
...
Chapter \ref{ch:deriving-unrestricted} motivates this analysis for the unrestricted type of language Old High German.
...
\begin{figure}[htbp]
  ...
  \begin{tabular}[b]{ccc}
      \toprule
      light head 2 & & relative pronoun 2 \\
      \cmidrule(lr){1-1} \cmidrule(lr){3-3}
      \begin{forest} boom
        [\tsc{d}P, s sep=20mm
            [\tsc{d}P,
...
The light head corresponds to \tit{d-er}, illustrated by the circle around the \tsc{d}P and the \tit{d-} under it and the circle around the \tsc{nom}P and the \tit{er} under it. The relative pronoun corresponds to \tit{d-er} too, illustrated by the circle around the \tsc{rel}P and the \tit{d-} under it and the circle around the \tsc{nom}P and the \tit{er} under it.
I draw a dotted circle around each constituent that is a constituent in both the light head and the relative pronoun.
The light head (the \tsc{d}P realized by \tit{d-er}) is syncretic with the relative pronoun (the \tsc{rel}P realized by \tit{d-er}).
As the two forms are entirely syncretic, either the light head or the relative pronoun can be deleted. For concreteness, I assume that the relative pronoun is deleted, and the headless relative surfaces in the internal and external case.
I illustrate this by drawing a dotted circle around each constituent that is a constituent in both the light head and the relative pronoun and by marking the content of the dotted circle for the relative pronoun gray.
...
The light head corresponds to \tit{d-en}, illustrated by the circle around the \tsc{d}P and the \tit{d-} under it and the circle around the \tsc{acc}P and the \tit{en} under it. The relative pronoun corresponds to \tit{d-er}, illustrated by the circle around the \tsc{rel}P and the \tit{d} under it and the circle around the \tsc{nom}P and the \tit{er} under it.
I draw a dotted circle around each constituent that is a constituent in both the light head and the relative pronoun.
