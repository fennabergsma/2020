...
The three assumptions I just introduced hold for all language types I discuss. In all language types, headless relatives are derived from light-headed relatives. For all language types, the deletion operation requires constituent containment. And in all language types, the relative pronoun consists of the features of the light head plus at least one additional feature (here \tsc{rel}).
...
Now compare the syntactic structures of the German accusative pronoun in \ref{ex:german-sie-spellout-rep} and the Khanty one in \ref{ex:khanty-luw-el-spellout-rep}. The feature content is the same (except for the feminine feature, which does not play a role here), but the syntax look different.
This change in syntax is a direct consequence of the lexical entries that are available within the language.
...
The different lexical entries lead to differences in the syntax of the light head and the relative pronoun.
Differences in the syntax of the light head and the relative pronoun lead to differences in whether or not one of them is contained in the other.
...
\ex.\label{ex:summary-differences-languages} lexical entries → syntax → containment → deletion → surface pronoun

The different language types appear by going through the chain in \ref{ex:summary-differences-languages} in the three different situations: (i) when the internal and external case match, (ii) when the internal case is the more complex case, and (iii) when the external case is the more complex case. An overview of these situation and whether or not a surface pronoun results from it is shown in Table \ref{tbl:overview-situations}.
...
In the unrestricted type of language, the lexical entries are such that there is a surface pronoun when the cases match, when the internal case is more complex and when the external case is more complex.
In the internal-only type of language, the lexical entries are such that there is a surface pronoun when the cases match and when the internal case is more complex but not when external case is more complex.
In the matching type of language, the lexical entries are such that there is a surface pronoun when the cases match but not when the internal case is more complex or when the external case is more complex.
...
For each language type I start with describing the kind of lexical entries they have, and I show the syntax that the light head and the relative pronoun have because of that.\footnote{
...
I show that the syntax I assume for the light head and the relative pronoun leads to the different patterns observed in the given language types.


\subsection{The internal-only type}\label{sec:basic-internal}

I start with the internal-only type of language. I suggest that the light head and the relative pronoun in this type of language form the syntactic structures shown in Figure \ref{fig:rel-lh-intonly-1}.
...
The light head (the \tsc{nom}P) is a constituent that is structurally contained within the relative pronoun (the \tsc{rel}P), so the light head can be deleted. I illustrate this by marking the content of the dashed circles for the light head gray.
...
I draw a dashed circle around each constituent that is a constituent in both the light head and the relative pronoun.
The light head (the \tsc{nom}P) still is a constituent that is structurally contained within the relative pronoun (the \tsc{rel}P), so the light head can be deleted. I illustrate this by marking the content of the dashed circles for the light head gray.
As the light head is deleted, the headless relative surfaces with the relative pronoun that bears the internal case.
...
The \tsc{nom}P of the light head is a constituent that is structurally contained within the relative pronoun, but the relative pronoun does not contain the feature \tsc{f}2 that forms an \tsc{acc}P.
The \tsc{nom}P of the relative pronoun is a constituent that is structurally contained within the relative pronoun, but the light head does not contain the feature \tsc{rel} that forms a \tsc{rel}P.
...
The comparisons between the light head and the relative pronoun in different cases correctly the derive the observed patterns in the internal-only type of language. An overview of the patterns is shown in Table \ref{tbl:overview-rel-light-mg}.
...
I continue with the matching type of language. I suggest that the light head and the relative pronoun in this type of language form the syntactic structures shown in Figure \ref{fig:rel-lh-matching}.
...
I draw a dashed circle around each constituent that is a constituent in both the light head and the relative pronoun.
The light head (the \tsc{nom}P) no longer is a constituent that is structurally contained within the relative pronoun (the \tsc{rel}P). Therefore, the light head cannot be deleted, which I illustrate by leaving the content of both dashed circles unfilled.
As none of the items is deleted, there is no grammatical headless relative possible.
...
In Figure \ref{fig:nom-acc-intonly-rep}, two different types of containment hold: feature containment and structural constituent containment.
...
With structural constituent containment, the \tsc{nom}P is a constituent that is structurally contained within the \tsc{rel}P. Therefore, the relative pronoun contains contains the light head.
...
Structural constituent containment is not only the requirement for deletion in headless relatives. It is also what seems to be crucial in NP ellipsis in general. \citet{cinqueforthcoming} argues that nominal modifiers can only be absent if they form a constituent with the NP. If they do not, they can also not be interpreted and ellipsis is ungrammatical. In what follows, I present his argument.
...
This observation regarding NP ellipsis provides independent evidence for my assumption that structural constituent containment is the crucial requirement for deletion of the light head or the relative pronoun in headless relatives.
...
I suggest that the first possible light head and the relative pronoun in this type of language form the syntactic structures shown in Figure \ref{fig:rel-lh-unres-1}.
...
So far, I proposed that the light head can be deleted when all of its features structurally form a constituent within the relative pronoun. This is impossible the other way around: all features of the relative pronoun can never structurally form a constituent in the light head, because the relative pronoun contains the feature \tsc{rel} that the light head does not. It seems that there is one case that defies this rule: syncretism.
In what follows I show a situation similar toe the missing \tsc{rel} feature: a syncretism between nominative and accusative case in Modern German. This situation leads me to propose a second type of constituent containment: formal constituent containment.
...
The light head (the \tsc{acc}P realized by \tit{s}) is a constituent that is formally contained within the relative pronoun (the \tsc{nom}P realized by \tit{s}), so the light head is deleted.
...
With this in mind, consider the second possible light head and the relative pronoun I suggest for the unrestricted type of language. I show them in Figure \ref{fig:rel-lh-unres-2}.\footnote{
...
In this chapter and in Chapter \ref{ch:deriving-unrestricted} (in which I work out the proposal for Old High German) I only discuss the situation in which the relative pronoun as a whole is a constituent that is structurally contained within the light head, and the relative pronoun is deleted.}
...
Chapter \ref{ch:deriving-unrestricted} motivates this analysis for the unrestricted type of language Old High German. It also shows that the two other languages types, discussed as Modern German and Polish, do not have a syncretism as described in \ref{ex:entry-d}, so the introduction of a second possible light head would not aid them.
...
I draw a dotted circle around each constituent that is a constituent in both the light head and the relative pronoun.
The relative pronoun (the \tsc{rel}P realized by \tit{XY}) is a constituent that is formally contained within the light head (the XP realized by \tit{XY}), so the relative pronoun is deleted.\footnote{
The same holds the other way around: the light head (the XP realized by \tit{XY}) is a constituent that is formally contained within the relative pronoun (the \tsc{rel}P realized by \tit{XY}). Therefore, either the light head or the relative pronoun can be deleted. I delete the relative pronoun here, as I discuss how it is possible for the relative pronoun to be deleted even though it has a feature less than the light head.
}
...
First consider only structural constituent containment, leaving formal constituent containment aside for now. The relative pronoun contains at least one feature more than the light head: the feature \tsc{rel}. The relative pronoun can not be deleted, because it is not a constituent that is structurally contained within the light head. In other words, a relative pronoun can never be deleted using structural constituent containment. This is also the point that I made before I introduced formal constituent containment in Section \ref{sec:basic-unrestricted}.
Now consider also formal constituent containment.


%something with a single constituent
%now we need to make two constituents
%then there is always the smaller constituent that is then contained in the relative pronoun, so then you have your internal wins pattern again

%or doing it differently: the only way in which it's problematic is when the light head is spread over two constituents, as in X. But then! There is also a smaller possible light head, namely the one of the other constituent and we're all good
