...
I come back to the two light heads.
...
Headless relatives in which the relative pronoun starts with a \tit{d}, such as in Old High German, seem to be linked to individuating or definite readings and not to generalizing or indefinite readings \citep[cf.][]{fuss2017}.
...
\exg. gihortut ir \tbf{thiu} \tbf{ih} \tbf{íu} \tbf{quad}\\
 ...
 `you listened to those things, that I said to you' \flushfill{Old High German, \ac{tatian} 165:6}\label{ex:ohg-acc-acc-rep}
...
\ex.
\ag. gihortut ir thiu \tbf{iu} \tbf{ih} \tbf{íu} \tbf{quad}\\
...
For other examples I do not know what the source structure is. Consider Table X.
...
%int>ext, int = acc, ext = nom, so extra light head, but individuation so light head expected
...
%ext>int, int = nom, ext = dat, so light head, and individuation so light head also expected
...
First internal more complex but individuating: there is still the \tit{d} in the relative pronoun, so this could also cause the definite interpretation

I do not have evidence for either of the light heas generating a different interpretation.
...
when it is the light head that is deleted, so the relative pronoun should be higher in the structure
...
two questions remain: (1) how do the cases end up down on the elh, and (2) what is the relation between the high and the low one?
