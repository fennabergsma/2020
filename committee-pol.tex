...
\chapter{Deriving the matching type}\label{sec:deriving-matching}
...
First, I discuss the relative pronoun. I decompose the relative pronouns into three morphemes, and I show which features each morpheme corresponds to.
Then I discuss the light head. I argue that Polish headless relatives are derived from a light-headed relative clause that do not surface in the language. I show that the features of the light head are spread over two morphemes. The fact that the light head has two morphemes is what ultimately leads Polish to be a matching type of language.
...
As I pointed out in Section \ref{sec:basic-idea}, I propose that the difference between Modern German and Polish headless relatives comes from whether the relative pronoun can delete the light head. This depends on whether the light head forms a constituent within the relative pronoun. That, in turn, depends on which constituents are formed to spell out the relative pronoun and the light head. The difference in spellout is the only difference between Modern German and Polish: the features that are spelled out are the same ones, namely the ones in \ref{ex:pol-fseq-rel}.
...
Clitics are used in non-stressed contexts. According to Franks, these clitics are not `real' clitics, since they syntactically behave like phrases. The deficiency is only just prosodic \pgcitep{franks2004}{22}.
}
...
Notice here that \tit{mu} has a unary bottom. Therefore, it can be inserted as the result of movement. That means that the lexical entry follows the existing structure and is spelled out as a suffix. This is how the correct order of \tit{je} and \tit{mu} comes about. I show how this works when I build the relative pronouns.

The morpheme \tit{go} is not used in the accusative neuter, but it is in the genitive. What I take away from that is that the morpheme \tit{go} needs to have \tsc{ind} as the lowest feature too, so that it can combine with the feature \tsc{an} if that is present and with the feature \tsc{cl} if \tsc{an} is absent. I give the lexical entry for \tit{go} in \ref{ex:pol-entry-go}.
...
I continue with the morpheme \tit{o}. Before I discuss its feature content, it discuss its phonology. Compare the animate relative pronouns to the masculine demonstratives in Table \ref{tbl:pol-rel-dem-o}.
...
\begin{table}[htbp]
  ...
  \begin{tabular}[b]{ccc}
    ...
    \bottomrule
  \end{tabular}
  \label{tbl:pol-rel-dem-o}
\end{table}

The \tit{k} /k/ combines with the \tit{o} /ɔ/, and the \tit{t} /t/ combines with the \tit{e} /ɛ/. I analyze this change as a phonological process, in which the vowel changes depending on the consonant. Specifically, I argue that the \tit{o} /ɔ/ in the relative pronoun is underlyingly an \tit{e} /ɛ/. The /ɛ/ becomes a /ɔ/ when it follows a /k/. I informally describe the situation in \ref{ex:e-o-phon-kt}.

\ex. ɛ ⇔ ɔ k$\_$\label{ex:e-o-phon-kt}

The description in \ref{ex:e-o-phon-kt} can be reformulated in more phonological terms. The consonant /t/ is coronal (i.e. front for consonants), and the vowel /ɛ/ is a front vowel. The consonant /k/ is dorsal (i.e. back for consonants), and the vowel /ɔ/ is a back vowel. The /ɛ/ changes into a /ɔ/ when it follows a dorsal consonant.

\ex. ɛ ⇔ ɔ [\tsc{dors}]$\_$\label{ex:e-o-phon-feat}

The vowel /ɛ/ is attested in another paradigm, namely in the genitive and dative of the inanimate relative pronouns. I put them besides the animate relative pronouns in Table \ref{ex:pol-rels}.
...
Here the /ɛ/ follows the /t͡ʂ/. This is in line with my generalization that dorsal consonants cause the vowel change. /t͡ʂ/ is a coronal consonant and not a dorsal, so it does not change.
Note that there is an \tit{o} /ɔ/ in the nominative and accusative. This is a different /ɔ/ than the one in the animate relative pronouns. It namely also spells out case features, which I argue later in this section the underlying /ɛ/ does not do. It namely also appears after the \tit{t} in the neuter nominative and accusative singular demonstrative.\footnote{
I assume that the change from \tsc{c} to \tit{cz} is I palatalization as a consequence of the combination of /t͡s/ and /ɛ/ \pgcitep{swan2002}{26}.
}

This brings me to the feature content of the \tit{e}. I argue that this is the \tit{je} from discussion on the morphemes \tit{go} and \tit{mu}.
The presence of the \tit{j} is phonology.

\tit{Je}-pronouns are weak pronouns. In \ref{ex:pol-jego-init} given an example in which \tit{jego} `3\tsc{sg}.\tsc{m}.\tsc{acc}' appears sentence-initially.

\exg. Jego jednego nie widzȩ.\\
 3\tsc{sg}.\tsc{m}.\tsc{acc} alone not see.1\tsc{sg}\\
 `Him, alone I don’t see.' \flushfill{Polish, \pgcitealt{swan2002}{157}}\label{ex:pol-jego-init}

I conclude from this that \tit{e}/\tit{o} contains the features \tsc{ref} and Σ. I add to this the features \tsc{cl} and \tsc{an} that I already assigned to \tit{e} earlier in this section (then still referring to it as \tit{je}), gives the lexical entry for \tit{e} in \ref{ex:pol-entry-e}.
...
This leaves the morpheme \tit{k} of the relative pronoun. I argue that \tit{k} spells out six features: \tsc{wh}, \tsc{rel}, \tsc{dx}\scsub{1}, \tsc{dx}\scsub{2}, \tsc{an} and \tsc{ind}. I go through them one by one.

I start with the operator features \tit{wh} and \tsc{rel}. Consider Table \ref{tbl:pol-rel-dem}, repeated from Table \ref{tbl:pol-rel-dem-o}.

\begin{table}[htbp]
  \center
  \caption{Polish relative pronouns and demonstratives \pgcitep{swan2002}{160,171} (repeated)}
  \begin{tabular}[b]{ccc}
    \toprule
              & \tsc{rel}.\tsc{an} & \tsc{dem}.\tsc{m} \\
    \cmidrule{2-3}
...
The relative pronouns listed are \tsc{wh}-pronouns, and they are also used as interrogatives in Polish. Therefore, just like the Modern German \tit{w}, the Polish \tit{k} spells out the features \tsc{wh} and \tsc{rel}.
...
I end with the gender and number features. Consider Table \ref{tbl:pol-rels-rep}, repeated From Table \ref{tbl:pol-rels}.
...
\begin{table}[htbp]
  ...
  \begin{tabular}[b]{ccc}
    ...
    \bottomrule
  \end{tabular}
  \label{tbl:pol-rels-rep}
\end{table}

The first consonant alternates depending on gender: the animates start with a \tit{k} and the inanimates start with a \tit{c(z)}. I conclude from this that the \tit{k} contains the feature \tsc{an}.\footnote{
It should be noted that numerous \tsc{wh}-elements in Polish start with a \tit{k}. I give some examples in \ref{ex:pol-wh-k}.
...
The animate \tsc{wh}-pronouns seems to pattern with the other \tsc{wh}-pronouns to the exclusion of the inanimate one. I return to this point during the discussion of how the relative pronoun is built. I show how it can be captured that the same \tit{k} is inserted in \tsc{wh}-pronouns used for animates, locations and references to time, and that inanimates receive a different spellout.

I also show how I model that the \tsc{wh}-pronouns alternate their vowel depending on animacy and that demonstratives do not.
}
Additionally, since the relative pronouns do not have a morphological plural, I assume that \tit{k} contains the feature \tsc{ind}.

In sum, the morpheme \tit{k} realizes the features \tsc{wh}, \tsc{rel}, \tsc{dx}\scsub{1}, \tsc{dx}\scsub{2}, \tsc{an} and \tsc{ind}.
...
In what follows, I construct the Polish relative pronouns. I follow the same functional sequence as I did for Modern German. The outcome is different because of the different lexical entries Polish has.

Starting from the bottom, the first two features that are merged at \tsc{ref} and Σ, creating a ΣP.
The syntactic structure forms a constituent in the lexical tree in \ref{ex:pol-entry-e-rep1}, repeated from \ref{ex:pol-entry-e}, which corresponds to the \tit{e}.

\ex.\label{ex:pol-entry-e-rep1}
\begin{forest} boom
  [\tsc{an}P
      [\tsc{an}]
      [\tsc{cl}P
...
Therefore, the ΣP is spelled out as \tit{e}, which I do not show here.

The features \tsc{cl} and \tsc{an} are merged and spelled out in the same way.
First, the feature \tsc{cl} is merged, and a \tsc{cl}P is created.
The syntactic structure forms a constituent in the lexical tree in \ref{ex:pol-entry-e-rep1}.
Therefore, the \tsc{cl}P is spelled out as \tit{e}.
Then, the feature \tsc{an} is merged, and a \tsc{an}P is created.
The syntactic structure forms a constituent in the lexical tree in \ref{ex:pol-entry-e-rep1}.
Therefore, the \tsc{cl}P is spelled out as \tit{e}, shown in \ref{ex:pol-spellout-e-rel}.
...
The next feature in the functional sequence is the feature \tsc{ind}. This feature can not be spelled out as the other ones before. The feature \tsc{ind} is merged, and a \tsc{ind}P is created. This syntactic structure does not form a constituent in the lexical tree in \ref{ex:pol-entry-e-rep1}. There is also no other lexical tree that contains the syntactic structure as a constituent.
Therefore, there is no succesfull spellout for the syntactic structure in the derivational step in which the structure is spelled out as a single phrase (\ref{ex:spellout-algorithm-phrasal-rep} in the Spellout Algorithm, repeated from \ref{ex:spellout-algorithm} in \ref{ex:spellout-algorithm-rep}).
...
The second movement option in the Spellout Algorithm is moving the complement, as described in \ref{ex:spellout-algorithm-comp}. In this case, the complement of \tsc{ind}, the \tsc{an}P, is moved to the specifier of \tsc{ind}P. The result of this movement operation in shown in \ref{ex:pol-spellout-e-ind}.
...
The \tsc{ind}P is a different constituent now. It still contains the feature \tsc{ind}, but it no longer contains the \tsc{an}P. The syntactic structure forms a constituent in the lexical tree in \ref{ex:pol-entry-go-rep1}. Therefore, the \tsc{ind}P is spelled out as \tit{go}.
...
The next feature in the functional sequence is the feature \tsc{dx}\scsub{1}. The story for this feature as its story in Modern German.
...
Therefore, in a second workspace, the feature \tsc{dx}\scsub{1} is merged with the feature \tsc{ind} (the previous syntactic feature on the functional sequence) into a \tsc{prox}P. The lexicon is checked for a lexical tree that contains the phrase \tsc{prox}P that contains \tsc{dx}\scsub{1} and \tsc{ind}. This syntactic structure does not form a constituent in any of the lexical trees in the language's lexicon.\footnote{
I actually assume that the lexical entry for the Polish demonstrative stem \tit{t} is match here. I show it in \ref{ex:pol-entry-t}.

\ex.\label{ex:pol-entry-t}
\begin{forest} boom
  [\tsc{med}P
      [\tsc{dx}\scsub{2}]
      [\tsc{prox}P
...
The captures the observation that \tit{t} does not change depending on gender. I do not discuss it in the main text, because it is overridden by the lexical entry \tit{k} later on in the derivation.
}

Therefore, the feature \tsc{dx}\scsub{1} combines not only with the feature merged before it, but with a phrase that consists of the two features merged before it: \tsc{ind} and \tsc{an}. This syntactic structure forms a constituent in the lexical tree in \ref{ex:pol-entry-k-rep}, repeated from \ref{ex:pol-entry-k}, which corresponds to \tit{k}.\footnote{
The lexical entry for the inanimate differs from the animate in that it contains the feature \tsc{cl} instead of \tsc{an}. I give the lexical entry for \tit{c} in \ref{ex:pol-entry-c}.

\ex.\label{ex:pol-entry-c}
\begin{forest} boom
  [\tsc{rel}P
      [\tsc{rel}]
      [\tsc{wh}P
  ...
  {\draw (.east) node[right]{⇔ \tit{c}}; }
\end{forest}

check place/manner/amount/time
explain pointer
...
time > amount > manner > place > person > thing

kiedy > ile > jak > gdzie > kto > co

This way the proposal is similar to \citet{wiland2019}.
}
...
The next feature in the functional sequence is the feature \tsc{dx}\scsub{2}. The story for \tsc{dx}\scsub{2} is similar to its story in Modern German.
