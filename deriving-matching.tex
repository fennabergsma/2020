...
Moreover, \tit{cz} is not a primary consonant but a derived one \pgcitep{swan2002}{23}. The consonants \tit{cz} and \tit{c} are derived from \tit{k}.
...
Under this analysis, Polish only has the morphemes \tit{k}, \tit{o} and \tit{i} that can be observed in the animate plus an \tit{j} that is present throughout the whole paradigm in the inanimate.

The next question is how the sequence \tit{k-j-i} becomes \tit{czy} in the instrumental, and the sequence \tit{k-j-o} becomes \tit{cze} in the genitive and dative, as shown in Table \ref{tbl:pol-rps-underl-real}.
...
Under this analysis, \tit{czyj} `whose' is underlyingly \tit{k-j-i-j}.
}
...
\tit{c} before \tit{e/y} → \tit{cz}

I start with the change of \tit{j} + \tit{k} to \tit{c}. Consider the paradigm for the singular of \tit{lampa} `light' and the singular of \tit{córka} `daugther' in Table \ref{tbl:pol-jk-to-c}. The stem and the suffixes are identical in both paradigms, except for in the dative.\footnote{
Notice also the change from \tit{y} to \tit{i} in the genitive after the \tit{k}-stem \tit{córk}.
}
...
I continue with the change of \tit{j} + \tit{o} to \tit{e}.

\ex. moje-story

I continue with the change of \tit{j} + \tit{i} to \tit{y}.
...
One context in which the \tit{o} becomes an \tit{e} is in the paradigm of the strong personal pronoun that starts with \tit{je}. Consider the accusative and dative masculine forms in Table \ref{tbl:pol-prons}.\footnote{
...
} I gave the surface form and what I analyze as the underlying form. The \tit{o} changes into an \tit{e} because it follows the \tit{j}.

\begin{table}[htbp]
  \center
  \caption{3\tsc{sg}.\tsc{m} personal pronouns \pgcitep{swan2002}{156}}
  \begin{tabular}[b]{ccc}
    \toprule
              & surface  & underlying  \\
    \cmidrule{2-3}
...
In Table \ref{tbl:pol-datives}, I show three forms in which there is a syncretism between the neuter and the masculine in the dative case. This shows that the syncretism is not specific for the relative pronoun (see Table \ref{tbl:pol-rps}).
...
In Table \ref{tbl:pol-datives}, the complete pronouns are syncretic. I set up a system that can derive the syncretism between the two genders. Doing this allows me to establish which features the morpheme \tit{mu} spells out.
...
In what follows, I show how I can derive the syncretisms for the forms in Table \ref{tbl:pol-datives}. Different from \tit{jullie}, these forms consists of three morphemes: \tit{j}/\tit{ni}/\tit{t}, \tit{o} and \tit{mu}.
I put the morphemes \tit{j}, \tit{ni} and \tit{t} aside, assuming that they spell out structure that differs per morpheme.
The structure that the combination of \tit{o} and \tit{mu} spell out is the syntactic structure of an extra light head, which I give in \ref{ex:pol-elh-dat-fseq}.
...
In \ref{ex:pol-spellout-o-an}, I give the syntactic structure of a masculine form. The syntactic structure forms a constituent within the lexical tree in \ref{ex:pol-entry-o}, and the structure can be spelled out as \tit{o}.
...
In \ref{ex:pol-spellout-o-cl}, I give the syntactic structure of a masculine form. The syntactic structure forms a constituent within the lexical tree in \ref{ex:pol-entry-o}, and the structure can be spelled out as \tit{o}.
...
What follows from this is that the lexical trees for the suffix \tit{mu} should contain all features in \ref{ex:pol-elh-dat} that are not spelled out by \tit{o} so far. These are the feature \tsc{ind} and all case features up to the dative. I give the lexical entry for \tit{mu} in \ref{ex:pol-entry-mu}.
...
Notice here that \tit{mu} has a unary bottom. Therefore, it can be inserted as the result of movement. That means that the lexical entry follows the existing structure and is spelled out as a suffix. This is how the correct order of \tit{o} and \tit{mu} comes about. I show how this works when I construct the extra light head later on in this section.
...
Starting from the bottom, the first two features that are merged \tsc{ref} and Σ, creating a ΣP.
The syntactic structure forms a constituent in the lexical tree in \ref{ex:pol-entry-o}, which corresponds to the \tit{o}.
Therefore, the ΣP is spelled out as \tit{o}, which I do not show here.
...
In this section I have argued that the extra light head in Polish is consists of the \tit{o} and a suffix. This means that I propose the light-headed relative that headless are derived from is of the type I show in \ref{ex:pol-elh-rp}
...
In the previous section I showed that the extra light head in Polish corresponds to two of the morphemes of the relative pronoun, namely the \tit{o} and the suffix \tit{go} or \tit{mu}. In this section, I show that the features that are not part of the extra light head are spelled out by the morpheme \tit{k}.
...
The features that are not contained in the extra light head are \tsc{c}, \tsc{dx}\scsub{1}, \tsc{dx}\scsub{2}, \tsc{wh} and \tsc{rel}. In addition, I propose that \tit{k} also spells out the features \tsc{ind}, \tsc{an} and \tsc{cl}.

In sum, I argue that Polish relative pronouns have the structure shown in \ref{ex:pol-rel}.
...
I continue with the feature \tsc{c}. Relative pronouns can be coordinated and focused, so they are strong.

I continue with the deixis features. I follow Baunaz/Lander and specifically for Polish \citet{wiland2019} in that indefinite demonstratives are contained in \tsc{wh}-elements.
...
In sum, the morpheme \tit{k} realizes the features \tsc{wh}, \tsc{rel}, \tsc{dx}\scsub{1}, \tsc{dx}\scsub{2}, \tsc{c}, \tsc{ind}, \tsc{an} and \tsc{cl}.
...
The pointer is situated above the \tsc{c}P. That means that if there is no animate feature in the structure, the \tsc{c} can also not be spelled out with \tit{k}. What follows is that there is another morpheme necessary that contributes the feature \tsc{c}. I propose that this is \tit{j} which forms strong pronouns. This is what causes the phonological processes described in Section \ref{sec:pol-elh}.
}
...
In what follows, I show how the Polish relative pronouns are constructed.

Until the feature \tsc{ind}, the derivation is identical to the one of the extra light. Therefore, I start at point at which the feature \tsc{c} is merged.
The derivation for this feature resembles the derivation of \tsc{c} in Modern German.
The feature is merged with the existing syntactic structure, creating a \tsc{c}P.
...
Therefore, in a second workspace, the feature \tsc{c} is merged with the feature \tsc{ind} (the previous syntactic feature on the functional sequence) into a \tsc{c}P. This syntactic structure does not form a constituent in any of the lexical trees in the language's lexicon.
Therefore, the feature \tsc{c} combines not only with the feature merged before it, but with a phrase that consists of the two features merged before it: \tsc{ind} and \tsc{an}. Also this syntactic structure does not form a constituent in any of the lexical trees in the language's lexicon.
Therefore, the feature \tsc{c} combines with a phrase that consists of the three features merged before it: \tsc{ind} \tsc{an} and \tsc{cl}. This syntactic structure forms a constituent in the lexical tree in \ref{ex:pol-entry-k}, which corresponds to the \tit{k}.
Therefore, the \tsc{c}P is spelled out as \tit{k}. The newly created phrase is merged as a whole with the already existing structure, and projects to the top node, as shown in \ref{ex:pol-spellout-cp}.
...
The next feature in the functional sequence is the feature \tsc{dx}\scsub{1}. The derivation for this feature resembles the derivation of \tsc{dx}\scsub{1} and \tsc{dx}\scsub{2} in Modern German.
The feature is merged with the existing syntactic structure, creating a \tsc{prox}P.
This structure does not form a constituent in any of the lexical trees in the language's lexicon, and neither of the spellout driven movements leads to a successful spellout.
Backtracking leads splitting up the \tsc{c}P from the \tsc{ind}P.
The feature \tsc{dx}\scsub{1} is merged in both workspaces, so with \tsc{c}P and and with \tsc{ind}P. The spellout of \tsc{dx}\scsub{1} is successful when it is combined with the \tsc{c}P.
It namely forms a constituent in the lexical tree in \ref{ex:pol-entry-k}, which corresponds to the \tit{k}.
The \tsc{prox}P is spelled out as \tit{k}, and the \tsc{prox}P is merged back to the existing syntactic structure.

The derivations for the features \tsc{dx}\scsub{2}, \tsc{wh} and \tsc{rel} happen the same way.
...
Backtracking leads splitting up the left branch from the \tsc{ind}P.
...
They namely form a constituent in the lexical tree in \ref{ex:pol-entry-k}, which corresponds to the \tit{k}.
The newly created is spelled out as \tit{k}, and they are merged back to the existing syntactic structure.
