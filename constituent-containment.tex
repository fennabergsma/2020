...
The fact that syncretism licenses deletion is not specific to the portion of the structure that corresponds to the \tsc{rel}P that contains the ϕP. Syncretism between different cases have the same effect. I illustrate this with an example from an internal-only language.

Suppose that there is a syncretism between the nominative and the accusative. That is, there is a lexical entry for the \tsc{acc}P which contains the feature \tsc{f}2 and the \tsc{nom}P, but not a more specific one that spells out the \tsc{nom}P on its own. In \ref{ex:nom-acc-syn}, I give the lexical entry, which spells out as /β/.
