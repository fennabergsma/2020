...
First I introduce the assumption that headless relatives are derived from light-headed relatives. The light head bears the external case, and the relative pronoun bears the internal case.
Then I introduce the core of the proposal: an element (i.e. relative pronoun or light head) is allowed to surface when it contains all constituent the other element (i.e. relative pronoun or light head) contains.
I present what I assume to be the internal structure of the light head and the relative pronoun, and I argue that the relative pronoun contains at least one feature more than the light head.
This set of assumptions derives that only a more complex case can surface and that there is no language of the external-only type.

In this dissertation so far, I stated that the relative pronoun appears in the case that wins the case competition. I have not been explicit about where the case competition takes place. In order to avoid introducing theoretical machinery just for case competition situations, I assume it takes place in syntax.
...
} In a light-headed relative clause, the head of a relative is not a full noun phrase, but it is a bit `lighter': there is no (overt) noun.
...
In my proposal, the difference between a light headed relative and a headless relative is that in a headless relative either the light head or the relative pronoun does not surface. The surfacing element is the one that bears the winning case, and the absent element is the one that bears the losing case. This means that what I have so far been glossing as and calling the relative pronoun is sometimes actually the light head. To reflect that, I call the surfacing element from now on the surface pronoun.

Table \ref{tbl:options-surface-pronoun} lists the two possibilities that I just laid out plus an additional one.
First, the surface pronoun can appear as the relative pronoun and bear the internal case. Second, the surface pronoun can appear as the light head and bear external case. The third option is that there is no grammatical form for the surface pronoun.
...
This last possibility arises namely in two of the three language types I described in Chapter \ref{ch:typology}.
Not all languages types allow both the internal and the external case to surface when either of them wins the case competition.
In an internal-only type language like Modern German, the more complex case is only allowed to surface when it is the internal case, and it is not allowed to surface when it is the external case. A matching type language like Polish allows neither the internal nor the external case to surface.
In these cases, it is neither the light head nor the relative pronoun that surfaces, but the construction is simply ungrammatical.

Table \ref{tbl:overview-rel-light} shows per language type whether it allows the relative pronoun and light head to surface if one case is more complex than the other.
The first two columns list the types of languages and the example for each of them that I discuss in this chapter.
The third column shows the situation in which the internal case is the most complex. The surface pronoun is the relative pronoun that bears the internal case. The fourth column shows the situation in which the external case is the most complex. The surface pronoun is the light head that bears the external case. The checkmark (✔) and asterix (*) indicate whether this is grammatical in the language or not.
...
What I have done in this section so far is reformulate the two descriptive parameters from Figure \ref{fig:two-parameters} into two other descriptive parameters. Whether the internal and external case are allowed to surface corresponds to whether the relative and light head are allowed to surface. Whether the the internal case is allowed to surface corresponds to whether the relative pronoun is allowed to surface. I show this in Figure \ref{fig:two-parameters-different}.
...
Reformulating these parameters is not just restating the generalization in different terms. Instead of indicating that the cases come from inside or outside of the relative clause, I identify the elements (i.e. the relative pronoun and the light head) that bear the cases. These two elements are crucial in my analysis. They namely do not only correspond to case features, but also to other features (having to do with number, gender, etc.). In this chapter I show that relative pronouns and light heads differ between the languages I describe. I illustrate how these differences in relative pronouns and light heads lead to the different language types in Figure \ref{fig:two-parameters-different}.

I propose that whether or not an element (i.e. relative pronoun or light head) is allowed to surface follows from the comparison between the relative pronoun and the light head.
...
\begin{figure}[H]
  ...
  \begin{tabular}[b]{ccc}
      ...
      \bottomrule
  \end{tabular}
   \caption {\tsc{acc} vs. \tsc{nom}: \tsc{acc}}
  \label{fig:acc-nom-structure}
\end{figure}

I formulated case containment in terms of featural containment. The accusative wins over the nominative because the accusative contains all features that the nominative contains. The imagine in Figure \ref{fig:acc-nom-structure} is also compatible with constituent containment. The accusative wins over the nominative because the accusative contains all constituents that the nominative contains. From now on I use constituent containment, as the difference turns out to be crucial later (in distinguishing the internal-only and the matching type).

I apply the containment reasoning to comparing the relative pronoun and light head. This time the most complex structure is not the winner of the case competition. Instead, the most complex structure is the element that is allowed to surface. When the relative pronoun contains all constituents of the light head, the relative pronoun is allowed to surface. When the light head contains all constituents of the relative pronoun, the light head is allowed to surface.

In order to be able to compare the relative pronoun and the light head, I zoom in on their syntactic structure. In Section Section \ref{sec:deriving-only-internal} to \ref{sec:deriving-nonmatching} I give arguments that support the structure I am assuming here. Figure \ref{fig:rel-lh-structure} gives a simplified representation of them.\footnote{
The structures in Figure \ref{fig:rel-lh-structure} are not base structures but derived one. I assume the base structure to be as in Figure \ref{fig:rel-lh-structure-base}.
...
The relative pronoun and the light head partly contain the same syntactic features. The features they have in common are case (\tsc{k}) and what I here call phi-features (ϕ). The relative pronoun and the light head differ from each other in that the relative pronoun in addition has a relative feature (\tsc{rel}).
...
From Figure \ref{fig:rel-lh-structure} it becomes immediately clear that the relative pronoun contains the light head, but not the other way around. The relative pronoun contains ϕ and \tsc{k} which the light head also contains. The light head does not contain \tsc{rel}. The relative pronoun contains all constituents of the light head, so the relative pronoun is allowed to surface. The light head does not contain all constituents of the relative pronoun, so the light head is not allowed to surface.
The type that I describe here is the internal-only type, the one that Modern German shows. However, not all language are of the internal-only type.
...
I start with the matching example Figure \ref{fig:rel-lh-nom-structure}. The relative pronoun appears in the nominative, and the light head does too.
...
\begin{figure}[H]
  ...
  \begin{tabular}[b]{ccc}
      ...
      \bottomrule
  \end{tabular}
   \caption {\tsc{acc} relative pronoun and \tsc{nom} light head}
...
Instead, the difference comes from how the relative pronouns and light heads are spelled out. In Sections \ref{sec:deriving-only-internal} to Section \ref{sec:deriving-nonmatching}, I show how I implement this idea. In a nutshell, the intuition is that matching languages like Polish package their features together differently in such a way that the constituents of the relative pronoun do not contain the constituents of the light head. Non-matching languages like Old High German have a syncretism between relative pronouns and light heads, and therefore also allow the light head to surface.
...
I return to the metaphor with the committee that I introduced in Section \ref{sec:possible-patterns}. I wrote that the committee learns who wins the case competition, and it can either approve this case or not approve it. I noted that the approval happens based on where the winning case comes from: from inside to the relative clause (internal) or from outside to the relative clause (external). This can now be rephrased in terms of elements that can surface or not: the internal case is approved when the relative pronoun is allowed to surface, and the external case is approved when the light head is allowed to surface. Whether these elements are allowed to surface depends on their syntactic structure: if each constituent of the light head is contained in a constituent of the relative pronoun, the internal case is allowed to surface, and if each constituent of the relative pronoun is contained in a constituent of the light head, the external case is allowed to surface. This means that case competition is not an independent process that needs to happen anymore. As the cases are part of the relative pronoun and the light head, case competition is simply part of the constituent comparison between the relative pronoun and the light head.

%Put shortly, the mechanism behind the comparison of light heads and relative pronouns is based on containment. This also holds for case competition.

Put shortly, case competition, which determines which case wins the case competition, relies the same mechanism as the process that determines whether an internal or external case is allowed to surface: constituent containment. Case differs from the other features in the two elements in that it can differ from sentence to sentence.
...
\begin{figure}[H]
  ...
  \begin{tabular}[b]{c}
      ...
      \bottomrule
  \end{tabular}
  \caption {Modern German \tsc{int}\scsub{nom} vs. \tsc{ext}\scsub{nom} = \tit{wer}}
...
\begin{figure}[H]
  \center
 \caption {Modern German \tsc{int}\scsub{acc} vs. \tsc{ext}\scsub{nom} = \tit{wen}}
  \begin{tabular}[b]{c}
      \toprule
...
\begin{figure}[H]
  \center
 \caption {Modern German \tsc{int}\scsub{nom} vs. \tsc{ext}\scsub{acc} ≠ \tit{wer}/\tit{ən}}
  \begin{tabular}[b]{c}
      \toprule
      \tsc{nom} relative pronoun \tit{w-e-r}
      \\
