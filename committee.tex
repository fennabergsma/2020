...
This section is structured as follows. First, I argue that Old High German headless relatives are derived from relative clauses headed by a demonstrative, i.e. light-headed relatives. In this analysis, the internal element is the relative pronoun, and the external element is the demonstrative. That means that in headless relatives with a more complex external case, it is actually the external demonstrative that surface as what we see as the relative pronoun. I decompose the internal and external element, and I show which morpheme corresponds to which features. The elements consist of two morphemes: a base part and a case part. I go through the examples in Table \ref{tbl:forms-ohg}, showing per situation how the base and case parts syntactically contain the other base and case parts.
...
I propose headless relatives are derived from relative clauses that have demonstratives as a head, i.e. light-headed relatives (\citealt{fuss2014} argue the same, but then for Modern German, \citealt{hanink2018} argues for non-pronounciation.\footnote{
In Modern German, the deletion of one of the elements is only possible when the cases match. In Section \ref{ch:discussion} I return to the point why Modern German does not have non-matching headless relatives that look like Old High German, although it still has syncretic relative pronouns and a demonstratives.
}).
In a light-headed relative, the head of a relative is not a full noun phrase, but it is a bit lighter: it only contains a demonstrative. Consider the light-headed relative in \ref{ex:ohg-double}. \tit{Thér} `\tsc{dem}.\tsc{sg}.\tsc{m}.\tsc{nom}' is the head of the relative clause, which is the external element. \tit{Then} `\tsc{rel}.\tsc{sg}.\tsc{m}.\tsc{acc}' is the relative pronoun of the relative clause, which is the internal element.
...
The difference between a light-headed relative and a headless relative is that in headless relatives, either the internal or the external is absent. The absent element is the one that has the least complex case. This brings me to the topic of optionality. In Old High German, the internal and external element can both surface, only the internal element can surface and only the external element can surface. If only one of the two elements surfaces, it is decided by whichever bears the most complex case. I assume that whether both or only one of the elements surfaces is determined by information structure. In \ref{ex:ohg-double}, the external element \tit{thér} `\tsc{dem}.\tsc{sg}.\tsc{m}.\tsc{nom}' is the candidate to be absent. However, it seems plausible that this is emphasized in this sentence and that is, therefore, not deleted. This sharply contrasts with headless relatives in Modern German, which are always ungrammatical when both the internal and external elements surface.

Support from the analysis that the Old High German headless relatives are derived from headed ones is their specific interpretation. \ref{ex:ohg-nom-acc-interpretation} is likely to mean `the single specific person whom they talk about' and not so much `whoever they talk about'.

(does Helmut also not say this?)
...
So, the internal element in Old High German is the relative pronoun, and the external element in Old High German is the demonstrative. Let us now take a closer look at the internal structure of the internal and external element. I illustrate how the internal base and the external base are identical, so they contain each other.

The pronouns consist of two morphemes: \tit{d} and suffix that differs per number, gender and case.\footnote{
\tit{d} can also be written as \tit{dh} and \tit{th}, \tit{ë} and \tit{ē} can also be \tit{e} and \tit{é}.
}
The two forms under discussion in this section are the masculine singular nominative and masculine singular accusative, marked gray in Table \ref{tbl:rel-dem-ohg}.

\begin{table}[H]\label{tbl:rel-dem-ohg}
 \center
 \caption {Relative and demonstrative pronouns in in Old High German \pgcitealt{braune2018}{339}}
  \begin{tabular}{cccc}
  \toprule
            & \ac{n}.\ac{sg}  & \ac{m}.\ac{sg}      & \ac{f}.\ac{sg}    \\
        \cmidrule{2-4}
  \ac{nom}  & d-aȥ            & \cellcolor{LG}d-ër  & diu               \\
  \ac{acc}  & d-aȥ            & \cellcolor{LG}d-ën  & d-ea/d-ia          \\
...
The \tit{d} morpheme corresponds to definiteness and anaphoricity. The suffix corresponds to number, gender and case.

% Not animate, but masculine. Human or so can be used for personal pronouns? These are more linguistic referents.

The suffixes also appear on adjectives and nouns, showing that these features also appear there (refer to the grammar).
...
I assume the features of the suffix are specifically: referential features, gender features, number features and case features. I adopt.. \tsc{ref}, \tsc{class}, \tsc{masc}, \tsc{ind}, \tsc{group} and \tsc{f1}, \tsc{f2}.

Now I can specify the lexical entries.
\tsc{masc} thing + cases separately in the lexical entry

then the \tit{d} morpheme, that corresponds to definiteness.
...
say something about the difference between \tsc{rel} and \tsc{dem}.




%check Pavel's way of talking through first phi, then D, then K.
...
I am not going to give a whole sentence. I only talk about the internal and external element in there.

The focus of this dissertation is on what the internal and the external element look like for them to be able to surface, and not on the exact structural configuration in which this happens. I do not commit to any analysis here, but to make things concrete, I could see this working in Cinque's story.

He says that there are always two heads in languages. The internal element c-commands the external element, because it is in the specifier of the specifier (Kayne).
...
When things become definite, the \tsc{ext} from below there is moved over the relative clause.
