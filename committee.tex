...
At first sight, it looks like headless relatives can be derived from this light-headed relative. This is exactly what \citet{hanink2018} does. She argues that the feature content of the light head and the relative pronoun is the same. Therefore, the light head is by default deleted. Only if the light head carries an extra focus feature it surfaces.

I take a slightly different approach. I assume that the demonstrative \tit{den} in \ref{ex:mg-wh-for-light-headed} has different features from the relative pronoun \tit{wen}. I assume that the demonstrative spells out features having to do with definiteness and that the relative pronoun spells out features having to do with free choice. This could also be an explanation for why certain German speakers do not allow for the combination of this demonstrative and relative pronoun and prefer the \tsc{d}-pronoun as relative pronoun in light-headed relatives instead. Their main objection was the reported clash in meaning between the \tsc{wh}-pronoun and the \tsc{d}-pronoun.

I stress the interpretation of headless relatives in Modern German, because the acceptability of the construction seems to correlate with one of its possible interpretation. Headless relatives can be combined with the the particle \tit{auch immer} `ever'.
...
This interpretation seems not very compatible a definite \tsc{d}-pronoun as \tit{den} in \ref{ex:mg-wh-for-light-headed}. I suggest that the light head is instead a weak demonstrative \citep{schwarz2009}.
...
I decompose the relative pronouns in three morphemes: the \tit{w}, the \tit{e} and the final consonant. I splits the light heads up into two morphemes: \tit{ə} and the final consonant.
...
The \tit{w} combines with the same endings as the \tit{d} does in demonstratives (or relative pronouns in headed relatives) in Modern German. Table \ref{tbl:mg-paradigm-dem} gives the singular forms in three gender and three cases.\footnote{
...
I adopt the gender and number that are distinguished by \citet{harley2002} for pronouns: \tsc{class}, \tsc{masc} and \tsc{ind}. The feature \tsc{class} refers to gender features, which is neuter if it is not combined with any other features. Combining \tsc{class} with the feature \tsc{masc} gives a masculine gender. The feature \tsc{ind} refers to number, which is singular if it is not combined with any other features.

\ex.
\a. [\tsc{class} + \tsc{masc} + \tsc{ind} + \tsc{f}1] = \tit{r}
\b. [\tsc{class} + \tsc{masc} + \tsc{ind} + \tsc{f}1 + \tsc{f}2] = \tit{n}

At this point, only the \tit{e} in the relative pronoun and the \tit{ə} in the light head are left. About the \tit{ə} I can be brief. I assume that the vowel correponds to the feature \tsc{ref}, which \citet{harley2002} claim all pronouns contain.

\ex. [\tsc{ref}] = \tit{ə}

This leaves the \tit{e} in the relative pronoun. I assume that, just like \tit{ə}, it contains \tsc{ref}. Besides that, it expressed features involving deixis. Modern German has a syncretic form for the proximal, the medial and the distal \pgcitep{lander2018}{387}. Therefore, it is not very informative. Looking at English, \tsc{wh}-pronouns are marked as at least medial. This can be shown for English. English has proximal and medial/distal demonstratives (evidence Lander/Haegeman).
...
The \tsc{wh}-pronoun is \tit{what} and \tit{whis} is ungrammatical.
...
It is the medial/distal form that is used on the \tsc{wh}-pronoun. The proximal is ungrammatical on \tsc{wh}-pronoun.
...
\ex. [\tsc{dx}\scsub{1} + \tsc{dx}\scsub{2}] = \tit{e}
...
Summarize how it is formed.
I combine the morphemes of animate relative pronoun in accusative case.
\tsc{ref} is complex?
