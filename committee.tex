...
The \tit{r} is the nominative masculine singular, so it spells out the features \tsc{ref}, Σ, \tsc{cl}, \tsc{an}, \tsc{ind} and \tsc{f}1. The \tit{n} is the accusative masculine singular, so it spells out the features that the \tit{r} spells out plus \tsc{f}2.
...
\ex. %to be revised, add the \tsc{dx}2
\begin{forest} boom
  [\tsc{rel}P
      [\tsc{rel}]
      [\tsc{wh}]
...
\exg. Uns besucht [\underline{r}], \underline{\tbf{wer}} \tbf{Maria} \tbf{mag}.\\
...
%here I discussed for German that the surface pronoun is the relative pronoun with extraposition facts. What can I say about Polish?
...
I start with the morpheme \tit{k}. Consider Table \ref{tbl:pol-rel-dem}. Leaving the vowel that follows the initial consonant aside (which I return to shortly), the \tit{k} and the \tit{t} combine with the same endings, which identifies both of them as a morpheme.
...
Before I continue with the other four features, I briefly return to the change in the vowel in Table \ref{tbl:pol-rel-dem}. The \tit{k} combines with the \tit{o} and \tit{i}, and the \tit{t} combines with the \tit{e} and \tit{y}. The \tit{e} and \tit{y} can also be found in another paradigm, namely following the \tit{cz} in inanimate relative pronouns. I put them besides the animate relative pronouns in Table \ref{ex:pol-rels}.
...
In my analysis, this change in a phonological process.

\ex.
\a. /k/ (voiceless velar plosive) /ɔ/ (open-mid back)
\b. /t/ (voiceless denti-alveolar plosive) /t͡ʂ/ (voiceless retroflex affricate) ɛ (open-mid front unrounded)

\ex.
\a. /k/ (voiceless velar plosive) /i/ (close front unrounded)
\b. /t/ (voiceless denti-alveolar plosive) /t͡ʂ/ (voiceless retroflex affricate) ɨ (close-mid advanced central unrounded)

\footnote{
Logically, there are a few more possibilities.
The first possible option is that different vowels have a different feature content, and that the consonant alternates as a phonological reflex. The idea is then that the masculine demonstrative and the inanimate relative pronoun pattern together to the exclusion of the animate relative pronoun. At first sight, this might seem counterintuitive. This changes when zooming in on the difference between \tsc{wh}-pronouns and demonstratives regarding the concept of gender. Demonstratives get their gender from the (possibly phonologically empty) head noun, and the gender is syntactic (i.e. it depends on the grammatical gender of the head noun). \tsc{wh}-pronouns (at least the ones in Table \ref{tbl:pol-rel-dem}) do not combine with a noun, so they get their gender from themselves, and their gender is semantic. Possibly, syntactic masculine gender contains one feature less than semantic animate gender: \tsc{masc} vs. \tsc{masc} + \tsc{an}. If the inanimate lacks both of these features, it can pattern with the masculine demonstrative to the exclusion of the animate relative pronoun.
The second possible option is that both the vowels (\tit{o} and \tit{e}) and the consonants (\tsc{k} and \tsc{t}) have a different feature content.
I leave investigating these options further for future research.
} I come back to the feature content of the vowel later. First I continue with the feature specification of \tit{k}.

Table \ref{tbl:pol-rels} shows that the first consonant (cluster) alternates depending on gender: the animates start with a \tit{k} /k/ and the inanimates start with a \tit{cz) /t͡ʂ/ or \tit{c} /t͡s/. I conclude from this that the \tit{k} contains the feature \tsc{an} (and the \tit{cz}/\tit{c} the feature \tsc{cl}). Additionally, since the relative pronouns do not have a morphological plural, I assume that \tit{k} contains the feature \tsc{ind}.

Finally for the morpheme \tit{k}, I turn to the deixis features. Unlike Modern German, Polish demonstratives are not marked for definiteness.
The demonstratives I gave in Table \ref{tbl:pol-rel-dem} are used as proximal and medial. There is a separate marker for the distal. The \tit{t} in Table \ref{tbl:pol-rel-dem} spells out these \tsc{dx}\scsub{1} and \tsc{dx}\scsub{2} features. As the \tit{t} is not present in the relative pronoun (compare e.g. \tit{temu} and \tit{komu}), I assume that \tit{k} spells out the deixis features itself.\footnote{
\citet{wiland2019} actually proposes that \tit{t} spells out a nominal base. I do not discuss the demonstrative any further, but I assume that the morpheme \tit{t} only corresponds to the feature \tsc{ind} in addition to \tsc{dx}\scsub{1} and \tsc{dx}\scsub{2}.
}
...
I continue with the morphemes \tit{go} and \tit{mu}. These morphemes contain at least case features.

Consider Polish \tit{je}-pronouns and \tit{ni}-pronouns. \tit{Je}-pronouns are used in emphatic or contrastive contexts and clause-initially. \tit{Ni}-pronouns are used after prepositions \pgcitep{swan2002}{156-157}. In unstressed situations, clitics, if available, can be used.

\begin{table}[htbp]
  \center
  \caption{Polish \tit{je}-pronouns}
\begin{tabular}{cccc}
  \toprule
          & \tsc{m} & \tsc{n} & \tsc{f} \\
          \cmidrule{2-4}
\tsc{acc} & j-e-go  & j-e     & j-ą     \\
\tsc{dat} & j-e-mu  & j-e-mu  & j-e-j   \\
  \bottomrule
\end{tabular}
  \label{tbl:pol-je-pronouns}
\end{table}


\begin{table}[htbp]
  \center
    \caption{Polish \tit{ni}-pronouns}
\begin{tabular}{cccc}
  \toprule
          & \tsc{m} & \tsc{n} & \tsc{f} \\
          \cmidrule{2-4}
\tsc{acc} & ni-e-go & ni-e    & ni-ą    \\
\tsc{dat} & ni-e-mu & ni-e-mu & ni-e-j  \\
  \bottomrule
\end{tabular}
  \label{tbl:pol-ni-pronouns}
\end{table}

Two observations can be made here. First, the suffix changes depending on the case. This indicates that the suffix spells out case features.
Second, the vowel does not change. This indicates--



This is a situation that can be achieved with the spellout algorithm.




Demonstratives and \tsc{wh}-pronouns differ independently with respect to gender.




First, I zoom in on \tit{mu}. Consider Table \ref{tbl:pol-mu-s}, which shows the morpheme \tit{mu} appears in the masculine and neuter gender on demonstratives and relative pronouns.

\begin{table}[htbp]
  \center
  \caption{Polish \tit{mu} in four contexts Swan, p. 171/}
  \begin{tabular}[b]{ccc}
    \toprule
                          & \tsc{dem} & \tsc{rel} \\
    \cmidrule{2-3}
    \tsc{an}/\tsc{masc}   & te-mu     & ko-mu     \\
    \tsc{inam}/\tsc{neut} & te-mu     & cze-mu    \\
    \bottomrule
  \end{tabular}
  \label{tbl:pol-mu-s}
\end{table}

For the demonstrative, not only the final marker is identical between the two genders, but the stem is the same too: \tit{te}. In other words, the demonstrative has a syncretic form for the neuter dative singular and the masculine dative singular. The forms for the relative pronoun are not syncretic here. I come back to this later in this section. What is important here is that the system is to be set up in such a way that it allows for this kind of syncretism. This allows me to establish which features \tit{mu} spells out.
...
\tit{Jullie} is syncretic between nominative, accusative and dative. It is specified for dative in the lexicon, because the dative contains the accusative and the nominative. The accusative second person plural in Dutch is spelled out as \tit{jullie} as well, because the \tsc{dat}P contains the \tsc{acc}P (Superset Principle), and there is no more specific lexical entry available in Dutch (Elsewhere Condition). It is important that the potentially unused features (so the \tsc{f}3 in case of the accusative) are at the top, so that the constituent that needs to be spelled out is still contained in the lexical tree.

I apply this reasoning to \tit{temu}, which is syncretic between the neuter and the masculine. First I consider the option that \tit{temu} is a single morpheme to illustrate the point about syncretism clearly. This means that the masculine feature needs to be the topmost feature, such that the constituent for the neuter is contained in the masculine. I give the lexical entry in \ref{ex:pol-entry-temu}.
...
\begin{forest} boom
  ...
  {\draw (.east) node[right]{⇔ \tit{temu}}; }
\end{forest}
\label{ex:pol-entry-temu}

\tit{Temu} corresponds to demonstrative features, the number feature \tsc{ind}, case features up to the dative, the \tsc{cl} feature that makes it a neuter and the \tsc{masc} feature that makes it a masculine. This lexical entry can be used for a dative masculine and a dative neuter: both correspond to a constituent contained in \ref{ex:pol-entry-temu}.

However, there are two problems with the structure in \ref{ex:pol-entry-temu}. The first problem is that considering \tit{temu} as a single lexical entry misses the point that \tit{mu} and \tit{te} also appears in other contexts. \tit{Mu} namely also appears in the dative relative pronoun, as shown in Table \ref{tbl:pol-mu-s}. \tit{Te} also appears in other forms of the demonstrative, e.g. the accusative masculine \tit{tego}, as shown in \ref{ex:pol-dem-rel}.
The second problem is that case features appear higher in the structure than gender features, as shown in the functional sequence in \ref{ex:pol-fseq-rel}.

In \ref{ex:pol-entry-temu-case-high} I solve the second problem by reversing the order of the case features and the gender features.
...
\begin{forest} boom
  ...
  {\draw (.east) node[right]{⇔ \tit{temu}}; }
\end{forest}
\label{ex:pol-entry-temu-case-high}

This solves the problem of going against the functional sequence, but it creates a new one: the lexical entry is not syncretic between the masculine and the neuter anymore. That is, there is no constituent contained in \ref{ex:pol-entry-temu-case-high} that contains the case features up to the dative but does not contain the \tsc{an} feature. The features that are in the way are the number and case features.
The other problem that still remains is that \tit{temu} is still analyzed as a single morpheme and it is not recognized that it consists of (at least) two: \tit{te} and \tit{mu}.

Both of these problems can be solved by letting the constituent has the feature \tsc{an} as its topmost feature (\tsc{an}P) move away. Both constituents then can receive their own spellout.

\ex.
\a.\label{ex:pol-entry-te}
\begin{forest} boom
  [
      [\tsc{an}P,name=tgt
          [\tsc{an}]
...
There are two constituents. The first constituent contains a number feature and case features up to the dative and corresponds to the morpheme \tit{mu}. The second constituent contains demonstrative features, gender features up to the
The animate feature is the highest feature, so that there is a constituent that spells out


So what needs to be in the lexicon is two lexical entries:
te
mu

notice something special about \tit{mu}. it has a unary bottom.





Both these problems can be solved by splitting \tit{temu} up in two morphemes: \tit{te} and \tit{mu}. I start with the lexical entry for \tit{te}.




This can be modeled in Nanosyntax by letting top-most feature of \tit{te} correspond be the feature \tsc{masc}.
...
The morpheme \tit{go} combines with \tit{te} and \tit{go}, and I assume it to start at the same point. Another
...
Consider the neuter and masculine
...
\begin{table}[htbp]
  \center
  \caption{Polish relative pronouns}
\begin{tabular}{ccc}
  \toprule
          & \tsc{m} & \tsc{n} \\
          \cmidrule{2-3}
\tsc{acc} & k-o-go  & cz-e-go \\
...
The animate and inanimate forms are no longer syncretic between the accusative and the dative. The \tsc{wh}-pronoun is \tit{cze} plus the suffix in inanimate, and \tit{ko} plus the suffix in the animate. This can mean three things: (1) \tit{cz} spells out different features than \tit{k} and \tit{e} spells out different features than \tit{o}, (2) \tit{cz} spells out different features than \tit{k}, and the change in vowel is a phonological process, or (3) \tit{e} spells out different features than \tit{o}, and the change in the consonant is a phonological process.

The phonological processes that I refer to known in Polish: as Baunaz \& Lander (2018a) point out, the suffix -o /o/ shifts into -e /e/ after a soft consonant ż- /ʒ/. This could work in two directions.

An indication into the direction of the third option is that \tsc{wh}-pronouns in Polish often start with a \tit{k}.

\ex.
\a. g-dzie = where at (g is the result of voicing)
\b. k-iedy = when
\b. k-tory = which one

Another instance of \tit{k} to \tit{cz} could be:

\ex.
\b. cz-yj = k-jyj 'whose'
\a. czyjego

Or this latter is another example of a \tsc{wh}-pronoun that is does not have a \tit{k} but \tit{cz}. There is also a \tsc{wh}-pronoun that starts with a \tit{j}:

j-ak = how

In dissertation, I go with the second option, namely that \tit{cz} and \tit{k} spell out different features and that the vowel changes according to the phonology. I leave determining whether it could be either of the other two options to future research.

The difference in features is the animacy: \tit{k} spells out \tsc{cl} and \tsc{masc} and \tit{cz} spells out \tsc{cl}.\footnote{
It is also possible for \tit{k} to spell out \tsc{masc} and \tit{cz} no gender features, but that does not work with my analysis.
}
...
For Modern German, I gave two reasons for not taking the existing light-headed relative as source of the headless relative. I show that only one of them holds for Polish, and I discuss why the other one does not hold in the same way.
...
The \tsc{dat}P and the \tsc{f}3 are placed between brackets. This is what distinguishes the dative extra light head from the accusative one. In the discussion that follows, I focus on the accusative extra light head. The same applies to the dative light head.

I need two lexical entries to spell out the accusative extra light head.
...
I give the dative light head for completeness.
...
First, the light-headed relative is derived from an existing light-headed relative, and the deletion of the light head is optional. Second, the light-headed relative is derived from a light-headed relative that does not surfaces in Modern German, and the deletion of the light head is obligatory.
For Modern German I concluded it was the second, and I proposed which features should be in there. Taking these features, I observe that the same holds for Polish: the extra light head is not attested as a light head in an existing light-headed relative in Polish.

For Modern German, I gave two reasons for not taking the existing light-headed relative as source of the headless relative. I show that only one of them holds for Polish, and I discuss why the other one does not hold in the same way.

to co cannot combine with \tit{kolwiek} `ever'.

\ex.
\ag. Jan śpiewa co -kolwiek Maria śpiewa.\\
Jan sings what ever Maria sings\\
`Jan sings everything Maria sings.'
\bg. Jan śpiewa to, co -kolwiek Maria śpiewa.\\
Jan sings \tsc{dem} what ever Maria sings\\
`John sings what Mary sings.' \flushfill{Polish, \pgcitealt{citko2004}{116}}

% the  incompatibility  of  light-headed  relatives  witheverfollowsfrom the fact thateverand the demonstrative are competing for the sameposition, namely the D0position

Going after citko's translation, one would assume that the meaning is also slightly different, just like it is in Modern German.

however, for a closely related language, Czech,
