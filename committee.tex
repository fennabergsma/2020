...
The difference between a light-headed relative and a headless relative is that in a headless relative either the light head or the relative pronoun does not surface. I propose headless relatives are derived from light-headed relatives
...
\begin{figure}[ht]
  \centering
    ...
    \begin{tikzpicture}[node distance=1.5cm]
      \node (question2) [question]
      {\tiny{delete relative pronoun?}};
          \node (outcome2) [outcome, below of=question2, xshift=-1.5cm]
          {matching};
              \node (example2) [example, below of=outcome2, yshift=0.25cm]
              {\scriptsize{e.g. Polish\\\phantom{x}}};
          \node (question3) [question, below of=question2, xshift=2cm, yshift=-0.5cm]
          {\tiny{delete light head?}};
              \node (outcome3) [outcome, below of=question3, xshift=-1.5cm]
              {internal-only};
                  \node (example3) [example, below of=outcome3, yshift=0.25cm]
                  {\scriptsize{e.g. Modern German\\\phantom{x}}};
    ...
    \caption{Delete \tsc{rel}/\tsc{lh} as parameters}
...
\begin{figure}[ht]
  ...
  \begin{tabular}[b]{ccc}
      ...
      \bottomrule
  \end{tabular}
   \caption {\tsc{nom} vs. \tsc{acc} after extraction ≠ \tsc{acc}}
...
For matching languages like Polish the intuition is that they package their features together differently from internal-only languages like Modern German. The packaging happens in such a way that the constituents of the relative pronoun do not contain the constituents of the light head. As a result, the relative pronoun cannot delete the light head anymore. In Section \ref{sec:deriving-matching}, I show that the different packaging of the features is a consequence of how the lexical entries for Polish light head and relative pronoun are specified in the lexicon.

What is special about unrestricted languages like Old High German is that their light heads and relative pronouns are syncretic. If there is no constituent containment, but the two forms are syncretic, one element can still delete the other. This means that the relative pronoun can not only delete the light head, but the light head can also delete the relative pronoun.
...
I continue with the left-most constituent of the relative pronoun: the \tsc{rel}P. This constituent is not contained in the light head, but it is syncretic with it. The ϕP is also spelled out as \tit{α}.
Each constituent of the relative pronoun is either also a constituent within the light head or it is syncretic with a constituent within the light head. Therefore, the relative pronoun can be absent. I illustrate this by marking the content of the dashed circles for the relative pronoun gray and the portion that is deleted by syncretism in a lighter shade of gray.

The fact that syncretism also licenses deletion is not specific to the portion of the structure corresponding to ϕ and \tsc{rel}. Syncretic cases can have the same effect, for instance inanimate nominatives and accusatives in Modern German. In Section \ref{sec:deriving-nonmatching} I give examples from that, and I show the detailed derivations for unrestrictive languages like Old High German.

The system I set up excludes the external-only type of language. An external-only type of language would be one in which the light head can delete the relative pronoun, but the relative pronoun cannot delete the light head. In my proposal, an element can the delete the other one if it contains all of the other's constituents. Relative pronouns always contain one more feature than light heads: \tsc{rel}. Therefore, it is impossible for a light head to contain all constituents of the relative pronoun. The light head namely does not even contain all features that the relative pronoun contains. If it does not meet the weaker requirement of feature containment, it is not possible to meet the stronger requirement of constituent containment.

I return to the metaphor with the committee that I introduced in Chapter \ref{ch:typology}. I wrote that first case competition takes place, in which a more complex case wins over a less complex case. This case competition can now be reformulated into a more general mechanism, namely constituent comparison. A more complex case corresponds to a constituent that contains the constituent that corresponds to a less complex case.

Subsequently, I noted that there is a committee that can either approve the winning case or not approve it. How I formulated it in Chapter \ref{ch:typology} is that the approval happens based on where the winning case comes from: from inside to the relative clause (internal) or from outside to the relative clause (external). I showed in this section that the approval relies on the same mechanism as case competition, namely constituent comparison. A relative pronoun corresponds to a particular set of constituents and so does the light head. If one contains all constituents the other contains, the former can delete the latter. When the element can delete the other element it is `allowed to surface'.

In other words, the grammaticality of a headless relative depends on several instances of constituent comparison. The constituents that are compared are those of the light head and the relative pronoun, which both elements bear their own case. Case is special in that it can differ from sentence to sentence within a language. Therefore, the grammaticality of a sentence can differ within a language depending on the internal and external case. The part of the light head and relative pronoun that does not involve case features is stable within a language. Therefore, whether the internal or external case is `allowed to surface' cannot differ within a language.
...
I give the light head - relative pronoun combination.

Modern German has two types of relative pronouns: \tsc{wh}-pronouns that appear in headless relatives and \tsc{d}-pronous that appear in headed relatives.\footnote{
At first sight it seems like \citet{fuss2014} discuss a counterexample to this generalization, namely headless relatives with \tsc{d}-pronouns. However, they claim that these headless relatives are actually light-headed relatives in which one of two syncretic elements is deleted by haplology.
}
\ref{ex:mg-wh-for-headless} is an example of a \tsc{wh}-pronoun in a headless relative, and \ref{ex:mg-d-for-headed} is an example of a \tsc{d}-pronoun in a headed relative.

\ex.
\ag. Jan umarmt \tbf{wen} \tbf{er} \tbf{mag}.\\
Jan hugs \tsc{rel}.\tsc{an}.\tsc{acc} he likes\\
`Jan hugs who he likes.'\label{ex:mg-wh-for-headless}
\bg. Jan umarmt den Mann \tbf{den} \tbf{er} \tbf{mag}.\\
...
Both of these relative pronouns seem to be compatible with light-headed relatives.

\ex.
\ag. Jan umarmt den \tbf{wen} \tbf{er} \tbf{mag}.\\
...
\bg. Jan umarmt den \tbf{den} \tbf{er} \tbf{mag}.\\
Jan hugs \tsc{d}.\tsc{m}.\tsc{sg}.\tsc{acc} \tsc{rel}.\tsc{m}.\tsc{sg}.\tsc{acc} he likes\\
`Jan hugs the man that he likes.'\label{ex:mg-d-for-light-headed}

At first sight, it seems like a possibility to assume that headless relatives are derived from the light-headed relative in \ref{ex:mg-wh-for-light-headed}. This is also exactly what \citet{hanink2018} does. In what follows, I motivate why I do not follow this proposal. The motivation comes from the interpretation of headless relatives.

Headless relatives differ from headed relatives in that they can be combined with the the particle \tit{auch immer} `ever'.

\ex.
\ag. Jan unarmt \tbf{wen} {\tbf{auch} \tbf{immer}} \tbf{er} \tbf{mag}.\\
Jan hugs \tsc{rel}.\tsc{an}.\tsc{acc} ever he likes\\
`Jan hugs whoever he likes.'\label{ex:mg-wh-for-headless-ever}
\bg. *Jan unarmt den Mann \tbf{den} {\tbf{auch} \tbf{immer}} \tbf{er} \tbf{mag}.\\
Jan hugs \tsc{d}.\tsc{m}.\tsc{sg}.\tsc{acc} man \tsc{rel}.\tsc{m}.\tsc{sg}.\tsc{acc} ever he likes\\
intended: `Jan hugs the man whoever he likes.'\label{ex:mg-d-for-headed-ever}

The sentence in \ref{ex:mg-wh-for-headless-ever} has two main interpretations (see \citet{s̆imík2020} for a recent elaborate overview on the semantics of free relatives). The first interpretation is described as universal-like and the second is described as definite-like. The universal-like interpretation corresponds to a universal quantifier: Jan hugs everybody that he likes. The definite-like interpretation corresponds to a definite description: Jan hugs the person that he likes.

\citet{s̆imík2020}{4} notes that some languages do not readily allow for the definite-like interpretation of headless relatives with an \tit{ever}-morpheme. There is no language documented that does not allow for the universal-like interpretation, but it does allow the definite-like interpretation. Along the same lines, informants have reported to me headless relatives with case mismatches seem to become more acceptable in the universal-like interpretation compared to the definite-like interpretation.
What I take away from that, is that the universal-like interpretation of headless relatives is the main interpretation to account for.

The headed relative in \ref{ex:mg-wh-for-light-headed} has as its light head \tit{den}. This is what \citet{schwarz2009} calls a strong demonstrative, which is used when there is an anaphoric relation. Often there is a linguistic antecedent that is referred back to. I give an example in \ref{ex:mg-florian-strong}. The strong demonstrative is \tit{dem} that precedes \tit{Freund}, which refers back to the linguistic antecedent \tit{einen Freund}.
...
So, this element does not surface as light head, but it does surface somewhere else, namely as clitic. I assume that the clitic is syncretic with the light head in Modern German. In Section \ref{sec:deriving-matching} I come back to how they differ from each other.

\ex.
\ag. Hat r einen Motorrad?\\
 has \tsc{dem}.\tsc{nom}\scsub{weak} a motor bike\\
 `Does he have a motorbike?'
\bg. Ich habe n gesehen.\\
 I have \tsc{dem}.\tsc{acc}\scsub{weak} seen\\
 `I have been him.'
\bg. Ich folge m nach Hause.\\
...
Now I turn to Modern German relative pronouns. Table \ref{tbl:mg-paradigm-wh-rels} gives the animate and inanimate forms in three cases: nominative, accusative and dative. I decompose them in three morphemes: a \tit{w}, a vowel and a final consonant. For each morpheme I show which syntactic features they correspond to. Then I form the complete relative pronouns with them.
...
I start with the morpheme \tit{w}.


The \tit{w} combines with the same endings as the \tit{d} does in demonstratives in Modern German. Table \ref{tbl:mg-paradigm-dems} gives the singular forms in three gender and three cases.\footnote{
These demonstratives are also used as relative pronouns of headed or regular light-headed relative clauses. To keep them apart, I keep referring to the \tsc{d}-pronouns as demonstratives and only to the \tsc{wh}-pronouns as relative pronouns.
}\footnote{
Note here that the relative pronouns, unlike the demonstratives, do not have a feminine form for the relative pronouns in Table \ref{tbl:mg-paradigm-wh-rels}. Demonstratives also have plural forms (which are not given here), and relative pronouns do not. As far as I know, this holds for all relative pronouns in languages of the internal-only type (cf. also for Finnish, even though it has lots of morphology) and of the matching type. Relative pronouns in languages of the unrestricted type do inflect for feminine, as well as always-external languages. In Chapter \ref{ch:discussion} I return to this observation in relation with the always-external languages.
}
...
This identifies the \tit{d} and, more importantly for the discussion here, the \tit{w} as morpheme. The morpheme \tit{w} appears besides in headless relatives, also as interrogative pronouns. An example is given in \ref{ex:mg-interrogative}.
...
For now, this leaves the vowel to express features having to do with referentiality and deixis.
...
\ag. d-er Baum dort/\#hier\\
 \tsc{d}-\tsc{dem}\scsub{strong} tree there/here\\
 `that tree there'
\bg. d-ies-ər Baum hier/\#dort\\
 \tsc{d}-\tsc{emph}-\tsc{dem}\scsub{weak} tree here/there\\
 `this tree here'
