...
\begin{tabular}{c|ccc}
  \toprule
                      & \tsc{int}         & \tsc{ext}         & ✔/* \\
                      \cmidrule{1-4}
\tsc{int} = \tsc{ext} & \tbf{d}-\tbf{e}-r & \tbf{d}-\tbf{e}-r & ✔   \\
\tsc{int} > \tsc{ext} & \tbf{d}-\tbf{e}-n & \tbf{d}-\tbf{e}-r & ✔   \\
\tsc{int} < \tsc{ext} & \tbf{d}-\tbf{e}-r & \tbf{d}-\tbf{e}-n & ✔   \\
\bottomrule
\end{tabular}
\end{table}
...
\section{Deriving `non-matching'}

Old High German allows both internal and external to win. I propose headless relatives are derived from light-headed relatives. Internal elemnt is relative pronoun, external element is demonstrative.
