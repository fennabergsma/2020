...
If the internal non-case part contains the features [\tsc{a}] or [\tsc{a}, \tsc{b}], and the external non-case part contains only the features [\tsc{a}] or [\tsc{a}, \tsc{b}], the internal and the external are allowed to surface in the language. This is the pattern that corresponds to the `non-matching' type of language like Old High German.
If the internal non-case part contains the feature [\tsc{a}], and the external non-case part contains only the features [\tsc{a}, \tsc{b}], the external is allowed to surface in the language and the internal is not. This is the pattern that is crosslinguistically not attested. In Section \ref{sec:int-ext-elements} I show why this is the case.
If the internal non-case part contains the features [\tsc{a}, \tsc{b}], and the external non-case part contains only the feature [\tsc{a}], the internal is allowed to surface in the language and the external is not. This is the pattern that corresponds to the `non-matching --- internal only' type of language like Modern German.
...
\begin{table}[H]
  \caption{\tsc{int} < \tsc{ext}}
\begin{tabular}{c|ccc}
  \toprule
                & yes/no & \tsc{int}   & \tsc{ext}   \\
                \cmidrule{1-4}
Old High German & yes    & d-e-n & d-e-r \\
Modern German   & yes    & w-e-n & r     \\
Polish          & no     & komu  & tego \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[H]
  \caption{\tsc{ext} < \tsc{int}}
\begin{tabular}{c|ccc}
  \toprule
                & yes/no  & \tsc{int}  & \tsc{ext}  \\
                \cmidrule{1-4}
Old High German & yes     & d-e-r      & d-e-n      \\
Modern German   & no     & w-e-r      & n          \\
Polish          & no      & temu       & kogo       \\
\bottomrule
\end{tabular}
\end{table}
...
the internal case is more complex than the external case, and the internal non-case part is more complex than the external non-cas part
...
the external case is more complex than the internal case, but the external non-case part is not more complex than the internal non-case part
