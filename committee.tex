...
he light head bears the external case, and the relative pronoun bears the internal case, as illustrated in \ref{ex:light+rel}.
...
I propose that whether or not a light head or relative pronoun) can delete the other follows from the comparison between the light head and relative pronoun. Specifically, I compare the constituents within light heads and relative pronouns to each other. Light heads and relative pronouns do not only correspond to case features, but also to other features (having to do with number, gender, etc.). It differs per language how language organize these features into constituents. In this chapter, I illustrate how these different constituents within light heads and relative pronouns lead to the differences in whether or not the light head and the relative pronoun can be deleted, and therefore to different language types.
...
I describe a situation in which the feature containment requirement holds but the constituent containment requirement does not.
In Figure \ref{fig:acc-nom-structure-moved-out} I show the same picture as in Figure \ref{fig:acc-nom-structure} except for that the XP has moved out of the \tsc{acc}P.
...
I draw a dashed circle around the constituent that is a constituent in both the accusative and the nominative (\tsc{nom}P).
...
Consider the top node of the relative pronoun: \tsc{rel}P. This constituent is not a constituent in the light head: the light head has a constituent \tsc{nom}P, but it does not contain \tsc{rel} to make it a \tsc{rel}P.
...
The differences between the languages from changing the feature content of the light head and relative pronoun per language.\footnote{
...
Instead, the differences come from how the light heads and relative pronouns are spelled out. In Sections \ref{sec:deriving-only-internal} to Section \ref{sec:deriving-nonmatching}, I show how I implement this idea.

In matching languages like Polish, the light head cannot delete the relative pronoun and the relative pronoun cannot delete the light head. The intuition for this type of language is that they package their features together differently from internal-only languages like Modern German. The packaging happens in such a way that the constituents of the relative pronoun do not contain the constituents of the light head. As a result, the relative pronoun cannot delete the light head anymore. In Section \ref{sec:deriving-matching} I show that only the stronger requirement of constituent containment is able to distinguish the internal-only from the matching type of language, and that the weaker requirement of feature containment is not.
...
Consider Figure \ref{fig:rel-lh-syncretism1}, in which the relative pronoun deletes the light head.\footnote{
These structures cannot be derived from the base structures in \ref{ex:base-light-head} and \ref{ex:base-relative-pronoun}. The structures I give here are simplifications of a more complex situation for which I give the intuition here.
}
...
Then I argue that the light head of the light-headed relative that the headless relative is derived from is a weak demonstrative in the sense of \citealt{schwarz2009}.
...
I show that the relative pronoun in Modern German headless relatives is the surface pronoun.
