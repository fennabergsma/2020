...
The relative pronoun in Polish.
...
As I pointed out in Section \ref{sec:basic-idea}, I assume that the difference between Modern German and Polish headless relatives comes from whether the relative pronoun can delete the light head. This, in turn, depends on whether the light head forms a constituent within the relative pronoun. What remains stable between the two languages is which features are spelled out.
...
I start with the morphemes \tit{go} and \tit{mu}. B
...
\begin{table}[htbp]
  ...
  \begin{tabular}[b]{cccc}
    ...
    \cmidrule{2-3}
    \tsc{nom} & to      & ten     & ta      \\
    \tsc{acc} & te-go   & te-go   & tę      \\
    \tsc{dat} & te-mu   & te-mu   & tej     \\
    \bottomrule
  \end{tabular}
  \label{tbl:pol-dem}
\end{table}
...
\begin{table}[htbp]
  \center
  \caption{Polish animate and inanimate \tsc{wh}-pronouns}
  \begin{tabular}[b]{ccc}
    \toprule
              & \tit{inan} & \tsc{an} \\
    \cmidrule{2-3}
...
The difference in features is the animacy: \tit{k} spells out \tsc{class} and \tsc{masc} and \tit{cz} spells out \tsc{class}.\footnote{
