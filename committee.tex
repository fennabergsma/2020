...
In Chapter \ref{ch:case-competition-typology} I introduced two parameters. The main focus is the second parameter of the two, which is which cases are allowed to surface when they win the case competition.
...
\section{Rely on containment}

The second parameter is about which cases are allowed to surface. The goal is to explain why languages there are languages that allow internal and external, languages that allow only internal, but no languages that only allow only external. I propose that this depends again on containment, just as was the case for case competition.

Committee: what kind of information does the committee use to decide whether the internal, external or both are allowed to surface? It compares the non-case part of the internal and the external element.

Two containment relations: case and non-case.

Remember from Chapter \ref{ch:decomposition}, a more complex case wins over a less complex case because the former contains all features the latter contains. Concretely, the dative wins over the accusative because the dative contains all features the accusative contains, the dative wins over the nominative because the dative contains all features the nominative contains, and the accusative wins over the nominative because the accusative contains all features the nominative contains.

I propose that a containment relation is also the crucial factor that determines in whether a particular language allows the internal and external case to surface. It is not the containment relationship having to do with case, but the one that has to do with the non-case part. This raises the question what the non-case parts of the internal and external element are. Headless relatives namely only have a single overt element, the relative pronoun. In Section \ref{sec:int-ext-elements}, I show that the relative pronoun is the internal element, and I introduce another element, which is the external one.

I propose that a language allows the internal case to surface when it wins the case competition, if the non-case part of the internal element contains all features that the non-case part of the external element contains. The other way around works the same way: a language allows the external case to surface when it wins the case competition, if the non-case part of the external element contains all features that the non-case part of the internal element contains.

If the internal non-case part contains the features \tsc{a} and \tsc{b}, and the external non-case part contains only the feature \tsc{a}, the internal is allowed to surface in the language. If the internal non-case part contains the feature \tsc{a}, and the external non-case part contains only the features \tsc{a} and \tsc{b}, the external is allowed to surface in the language. If the internal non-case part contains the features \tsc{a} and \tsc{b}, and the external non-case part contains only the features \tsc{a} and \tsc{b}, the internal and the external are allowed to surface in the language.
...
The crucial difference between the containment of the case part and the containment of the non-case part is how much it can differ. Case differs from sentence to sentence: in one sentence the internal case is the dative, in the other it is the accusative or the nominative. Therefore, there are different winners in different sentences within the same language. The non-case part is stable within a language. In this chapter, I introduce what the non-case part the internal and external element looks like, and I argue per language what they look like.
