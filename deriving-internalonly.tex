...
\tit{Wer} `who' is the relative pronoun that Modern German uses in headless relatives. The language also has other relative pronouns, such as \tit{der} `who' \tit{welcher} `which'. In this dissertation, I focus on \tit{wer}, since it is the relative pronoun used in the headless relatives, and I do not discuss how this relative pronoun relates to the other relative pronouns.
}\footnote{
...
} In \ref{ex:mg-weak}, I give examples of the masculine accusative singular and masculine dative singular.\footnote{
I constructed the German sentences in this chapter myself. I checked their grammaticality with several native speakers.
}
...
I assume that a relation is established with the light head. The semantics of the headless relative as a whole is then an individual that has been picked from a set of alternatives \citep[cf.][]{caponigro2003}.
...
\citet{cardinaletti1994} split pronouns in three classes: strong pronouns, weak pronouns and clitics. Following the tests in \citet{cardinaletti1994} that distinguish the types from each other, the pronouns in \ref{ex:mg-weak} are neither strong pronouns nor clitics, and therefore, should be classified as weak pronouns.\footnote{For a different classification on the basis of different German dialects, see \citealt{weiss2015}.}
...
It seems that these examples are not grammatical for all speakers of Modern German. For these speakers, the pronouns are possibly clitics. I come back to this at the end of Chapter \ref{ch:deriving-matching} and in the discussion in Chapter \ref{ch:discussion}.
}
...
Relative pronouns do not express spatial deixis, but discourse deixis: it establishes a relation with an antecedent.
...
\subsection{Alternative segmentation of \tit{wer}}\label{sec:mg-w-er-cv}
...
Both \citet{picallo2008} and \citet{kramer2016} argue that number (\#) is hierarchically higher than gender (\tsc{cl} and \tsc{an}). Case (\tsc{k}) is agreed to be higher than number (\#) \citep[cf.][]{pittner1996}.
...
This next feature is the feature \tsc{an}, and am \tsc{an}P is created.
...
\citet{caha2019a} argue that there can only be a single feature overlap between the two phrases.
...
\ex.\label{ex:backtracking} \tbf{The logic of backtracking} \pgcitep{caha2019}{198}\\
...
\ex.\label{ex:multiple-merge} \tbf{Multiple Merge} \pgcitep{caha2019}{227}\\
...
It seems that light-headed relatives with inanimates, as in \ref{ex:mg-das-was}, are judged better than examples with animates \citep[cf.][ftn. 29]{hanink2018}. In turn, examples with different cases, as in \ref{ex:mg-wer-dem}, are judged worse than examples with matching cases, although they are still attested \citep[ftn. 6]{fuss2014}.
...
}\footnote{At least three speakers I consulted accept the example in \ref{ex:mg-den-wen}, but not every speaker of Modern German does.
...
\ex.\label{ex:mg-context-def}
\a. Context: Yesterday I met with two friends. I like one of them. The other one I do not like so much.
...
\exg. Ich lade [n] ein, \tbf{wen} \tbf{auch} \tbf{Maria} \tbf{mag}.\\
 1\ac{sg}.\ac{nom} invite.\ac{pres}.1\ac{sg}\scsub{[acc]} \tsc{elh}.\ac{an}.\ac{acc} {} \tsc{rp}.\ac{an}.\ac{acc} also Maria.\ac{nom} like.\ac{pres}.3\ac{sg}\scsub{[acc]}\\
...
None of the elements can be deleted via formal containment either. The relative pronoun \tit{wen} and the extra light head \tit{m} do not show any formal containment.
}
...
When the external case is the more complex one, neither the light head nor the relative pronoun is structurally contained in the other element. None of the elements can be deleted, and there is no grammatical headless relative possible.
