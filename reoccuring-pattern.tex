...
This chapter introduces the pattern that forms the focus of the first part of the dissertation. In Section \ref{sec:pattern-rels} I show that headless relatives adhere to the case scale in \ref{ex:case-scale-intro}.

\ex. \ac{nom} < \ac{acc} < \ac{dat}\label{ex:case-scale-intro}

Then I show that this pattern is not unique to headless relatives. It appears in more morphosyntactic and morphophonological phenomena. Section \ref{sec:impl-hier} discusses two implicational hierarchies that show the case ordering. The hierarchies concern agreement and relativization across languages. Section \ref{sec:case-morphology} shows that the case scale also shows up in phonological patterns. It can be observed in patterns of syncretism and in morphological containment.


\section{Case competition in headless relatives}\label{sec:pattern-rels}

As the name suggests, headless relatives are relative clauses that lack an (overt) head. The internal case, the case from the relative clause, and the external case, the case from the main clause, compete to surface on the relative pronoun. It is crosslinguistically shown that the two competing cases always adhere to particular case scale \citep[cf.][]{harbert1978,pittner1995,vogel2001,grosu2003,caha2019}. This scale is given in \ref{ex:case-scale}. Elements more on the right on this scale win over elements more on the left of this scale.\footnote{
...
It differs per language with how many of its arguments a predicate agrees. However, it is not random with which agreement takes place. Instead, there is a implicational hierarchy that is identical to the one observed for headless relatives: \ac{nom} < \ac{acc} < \ac{dat}.
...
It has widely been established that syncretism is restricted by the linear sequence \tsc{nom} --- \tsc{acc} --- \tsc{dat} \citep{baerman2005,caha2009,zompi2017,mcfadden2018,smith2019case}. That is, if one orders cases in this linear sequence, only contiguous regions in the sequence can be syncretic.
...
There is one pattern that does not target continuous regions, but non-contiguous ones: nominative and dative are syncretic and accusative is not. This pattern is not attested crosslinguistically. In other words, there is no ABA pattern (in which a form B intervenes between the two identically formed As) \citep{bobaljik2012}.

Table \ref{tbl:syncretisms} shows examples for each of these possible patterns. I give an example from a syncretism between nominative, accusative and dative from Dutch. The second person plural pronoun is \tit{jullie} `you.\tsc{pl}' is syncretic between nominative, accusative and dative.
...
\begin{table}[ht]
  \center
  \caption {Syncretism in \tsc{nom} --- \tsc{acc} --- \tsc{dat}}
...
This section shows a second way in which \tsc{nom} < \tsc{acc} < \tsc{dat} is reflected in morphology: formal containment. In some languages, the form that is used for the accusative literally contains the form that is used for the nominative. In turn, the forms for the dative contains the form for the accusative. I illustrate this phenomenon with examples from Khanty.

Khanty (or Ostyak) \pgcitealt{nikolaeva1999}{16} shows formal containment in some of its pronouns. Three examples are given in Table \ref{tbl:cont-khanty}.

The nominative form for the first person singular is \tit{ma} `I.\tsc{nom}'. The form for the accusative is \tit{ma:ne:m} `me'. This is the form for the nominative \tit{ma} plus the accusative marker \tit{-ne:m}. The form for the dative is \tit{ma:ne:mna} `me'. This is the form for the accusative \tit{ma:ne:m} plus the dative marker \tit{-na}. So, dative formally contains the accusative, and the accusative formally contains the nominative.
...
In sum, some languages morphologically look like \tsc{nom}-\tsc{acc}-\tsc{dat}. This exactly reflects the case scale \tsc{nom} < \tsc{acc} < \tsc{dat}.



\section{Excluding the genitive}\label{sec:genitive}

In the literature about headless relatives, the genitive is often discussed together with the nominative, accusative and dative \citep[cf.][]{harbert1978,pittner1995}. In this dissertation I restrict myself to cases that appear in all possible case competition combinations. The genitive does not fulfill that requirement, and is therefore excluded. I shortly discuss how the genitive is different from the three other cases.
...
\exg. ni waiht þiz \tbf{-ei} \tbf{gasehvun}\\
 not thing\scsub{[gen]} what.\ac{gen} -\tsc{comp} saw\scsub{[acc]}\\
...
If the internal case is genitive and the external case is accusative, the genitive wins as well. There are no attested examples in Gothic of genitives in case competition with nominatives or datives.
...
In Old High German might show some examples of case competition between genitives and accusatives and genitives and nominative. In these cases, the genitive always wins. No examples of datives against genitives are attested \citep{behaghel1923}.
...
This case scale is not only found in case competition in headless relatives. Implicational hierarchies regarding two morphosyntactic phenomena appear across languages. The first one concerns agreement. If a language shows agreement with datives, it also shows agreement with accusatives and nominatives. If a language shows agreement with accusatives, it also shows agreement with nominatives.
