...
\exg. Ich lade die Person ein, \textbf<3->{der} \textbf<3->{Maria} \textbf<3->{vertraut}.\\
 1\tsc{sg}.{\tsc{nom}} invite.\tsc{pres}.1\tsc{sg}{\scsub{[acc]}} the person {} \tsc{rel}.\tsc{sg}.\tsc{f}.{\tsc{dat}} Maria.{\tsc{nom}} trust.\tsc{pres}.3\tsc{sg}{\scsub{[dat]}}\\
...
\begin{enumerate}
  ...
  \begin{itemize}
      \onslide<4->{\item is stable across languages}
      \onslide<5->{\item \tsc{nom} < \tsc{acc} < \tsc{dat}}
  \end{itemize}
  \item whether the winner gets approved
  \begin{itemize}
      \onslide<6->{\item differs across languages}
      \onslide<7->{\item \tsc{int}/\tsc{ext} approved}
  \end{itemize}
\end{enumerate}
...
\onslide<8->{
\begin{itemize}
  \item \textbf<9>{illustrate the generalizations}\pause
...
\begin{frame}[t]{The winner of the competition in German (Vogel 2001)}

\pause

\center
\textbf<9->{\tsc{nom} <} \textbf<5->{\tsc{acc} < \tsc{dat}}
...
\begin{frame}[t]{\tsc{ext}\scsub{acc} vs. \tsc{int}\scsub{dat} ↛ \tit{ogo}/\tit{komu}}
