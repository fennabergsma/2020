...
The external case is dative as well, as the predicate \tit{vertrauen} `to trust' takes dative objects.
...
There is no grammatical way of saying this, so people try to make it work one way or the other by somehow repairing it. There are two ways of doing that: (1) they can take \tit{wer gutes tut} and \tit{wer mich verletzt} as clauses objects, (2) they add \tit{den} as an object and drop it at the same time. Notice that this crucially differs from the Old High German example with Jezus. In that one, there was a grammatical option.
}
...
I continue with the competition between dative and nominative. Following the case scale, the relative pronoun appears in the dative case and never in nominative. Following the internal-only requirement, when the dative case is the internal case, the sentence is grammatical.
...
I end with the competition between dative and accusative. Following the case scale, the relative pronoun appears in the dative case and never in accusative. Following the internal-only requirement, when the dative case is the internal case, the sentence is grammatical.
...
The two examples in which accusative and dative compete are highlighted in Table \ref{tbl:summary-polish-acc-dat}.
...
This gives the following table \ref{tbl:case-competition-table}.
...
The second aspect is whether the internal and the external case are allowed to surface when either of them wins the case competition. This differs across languages. There are four possibilities: (1) the internal and the external case are allowed to surface when either of them wins, (2) only the internal case is allowed to surface when it wins, (3) only the external case is allowed to surface when it wins, and (4) neither the internal nor the external case is allowed to surface.
...
Table \ref{tbl:possible-headless-relatives-case-competition} gives an overview of the patterns and whether they are attested. The top row sketches two different situations: one in which the internal case is the most complex ([\ac{int}]>[\ac{ext}]) and one in which the external case is the most complex ([\ac{ext}]>[\ac{int}]). The second row refers to the case in which the relative pronoun appears, which can be either the internal case (\ac{int}) or the external case (\ac{ext}).

\begin{table}[H]
  \center
  \caption{Possible patterns for headless relatives with case competition}
...
The row numbered as 1 in Table \ref{tbl:possible-headless-relatives-case-competition} corresponds to the first pattern, in which the relative pronoun is allowed to surface in the internal and the external case, when either of them wins the case competition.
The most left checkmark indicates that the internal case is allowed to surface in the internal case when the internal case is the most complex. This corresponds to the light gray marked cells in Table \ref{tbl:case-competition-table}.
The most right checkmark indicates that the external case is allowed to surface in the external case when the external case is the most complex. This corresponds to the dark gray marked cells in Table \ref{tbl:case-competition-table}.
In sum, all cells in Table \ref{tbl:case-competition-table} are grammatical.

The row numbered as 2 in Table \ref{tbl:possible-headless-relatives-case-competition} corresponds to the second pattern, in which the relative pronoun is only allowed to surface in the internal case, when it wins the case competition.
The checkmark indicates that the internal case is allowed to surface in the internal case when the internal case is the most complex. This corresponds to the light gray marked cells in Table \ref{tbl:case-competition-table}.
There is no checkmark in columns that correspond to the situation in which the external case wins the case competition, meaning that there is no grammatical result possible. This means that the dark gray marked cells in Table \ref{tbl:case-competition-table} are ungrammatical in this pattern.
In sum, the diagonal and the light gray marked cells in Table \ref{tbl:case-competition-table} are grammatical, the dark gray marked ones are not.

The row numbered as 3 in Table \ref{tbl:possible-headless-relatives-case-competition} corresponds to the third pattern, in which the relative pronoun is only allowed to surface in the external case, when it wins the case competition.
The checkmark indicates that the external case is allowed to surface in the external case when the external case is the most complex. This corresponds to the dark gray marked cells in Table \ref{tbl:case-competition-table}.
There is no checkmark in columns that correspond to the situation in which the internal case wins the case competition, meaning that there is no grammatical result possible. This means that the light gray marked cells in Table \ref{tbl:case-competition-table} are ungrammatical in this pattern.
In sum, the diagonal and the dark gray marked cells in Table \ref{tbl:case-competition-table} are grammatical, the light gray marked ones are not.

The row numbered as 4 in Table \ref{tbl:possible-headless-relatives-case-competition} corresponds to the third pattern, in which the relative pronoun is neither allowed to surface in the internal case nor in the external case.
There is no checkmark for this pattern, meaning that there is no grammatical result possible when the internal case is more complex than the external case or the other way around (i.e. when the internal case and the external case differ). This means that the light gray marked cells and the dark gray marked cells in Table \ref{tbl:case-competition-table} are ungrammatical in this pattern.
In sum, only the cells in the diagonal are grammatical.
...
I model the possibilities across the languages using two parameters: (1) does the language allow the internal case as a winner? and (2) does the language allow the external case as a winner? This gives me three possible language. The language that only allows the only external case to win is not generated.

If the first question is answered with no, all non-matching headless relatives are ungrammatical. If the first question is answered with yes, then a second question comes. By setting it up like this, it is impossible to get a language that only allows the external question, because you never reach the question. If you answer the second question with no, then you only allow internal. If you answer the second question with yes, you allow both internal and external.

You can see this flow chart as a committee that lets all the contesters go through two rounds. Round 1: int? if you are out then, you are not considered for the second question. if you get through the first round, then you get to the second question of the ext question.
...
Section \ref{sec:always-ext} discusses two languages that always let their relative pronouns in headless relatives surface in the external case: Old English and Modern Greek. In Section \ref{sec:typology} I extend the case competition typology from Section \ref{sec:summary-3-patterns} by adding the langauges without case competition. As I briefly mentioned, I do not know of any languages, whether extinct or alive, that lets the relative pronoun always surface in the internal case. I do not offer an explanation for why it is not attested, and I include this possibility in my typology.
...
Two example of languages that shows this pattern are Old English and Modern Greek. In this section I discuss the Old English data with examples from \citet{harbert1983}. The Modern Greek data I discuss is taken from \cite{daskalaki2011}. For all examples holds that I made glosses more detailed, and I added and modified translations.
...
This example shows that Old English is neither an instance of the pattern in Section \ref{sec:pattern-iii}, in which only the external case is allowed to surface, nor an instance an instance of the pattern in Section \ref{sec:pattern-i}, in which the internal case and external case are allowed to surface. Instead, as illustrated by Table \ref{tbl:no-case-competition-old-english}, the language does not have any case competition. The relative pronoun appears in the external case: the external case can be the most complex case, illustrated by the example in \ref{ex:oe-dat-nom}, marked here in light gray, or the external case can be the less complex case, illustrated by the example in \ref{ex:oe-acc-dat}, marked here in dark gray.
...
This example shows that Modern Greek is neither an instance of the pattern in Section \ref{sec:pattern-iii}, in which only the external case is allowed to surface, nor an instance an instance of the pattern in Section \ref{sec:pattern-i}, in which the internal case and external case are allowed to surface. Instead, as illustrated by Table \ref{tbl:no-case-competition-greek}, the language does not have any case competition. The relative pronoun appears in the external case: the external case can be the most complex case, illustrated by the example in \ref{ex:greek-acc-nom}, marked here in light gray, or the external case can be the less complex case, illustrated by the example in \ref{ex:greek-nom-acc}, marked here in dark gray.
...
In sum, Old English and Modern Greek are two languages that do not show any case competition. The relative pronoun always appears in the external case.


\subsection{A typology of headless relatives}\label{sec:typology}

This section provides a typological overview of headless relatives. I give all logically possible patterns and show with a diagram to what extend I can exclude the non-attested patterns.

In Section \ref{sec:pattern-i} to \ref{sec:pattern-iv} I four different patterns that are all derived from the same table. The relative pronoun that appears is the one that wins the case competition, as in Table \ref{tbl:case-competition-int-ext-typology}. The variation between the four arises from allowing either the internal case, the external care, both of them or neither of them to surface.
...
In this section I introduced two different ways of filling out the table. The first one is the one in which the relative pronoun appears in the internal case, as in Table \ref{tbl:no-case-competition-int-typology}
...
The second one is the one in which the relative pronoun appears in the external case, as in Table \ref{tbl:no-case-competition-ext-typology}
...
I incorporate the options to generate these different patterns into the diagram from Section \ref{sec:summary-3-patterns} in Figure
\ref{fig:typology}. I added to parameters. First, a language can either have case competition or not. If the language does, Table \ref{tbl:case-competition-int-ext-typology} is generated. For what follows, see Section \ref{sec:summary-3-patterns}.

If the language does not have case competition, the second parameter is whether the language lets the relative surface in the internal case or in the external case. Old English and Modern Greek are two examples of languages that let their relative pronouns in headless relatives appear in the external case. I am not aware of any language that lets its relative pronoun in headless relatives appear in the internal case. In this dissertation I do not offer an explanation for why this type of example should be absent. Future research should determine whether this pattern is actually attested, or whether this option should be excluded and how.
...
When the internal case and the external case differ (which is true for both options the top row indicates), the relative pronoun cannot appear in both the internal and external case. This excluded the possibility of having a checkmark at both \ac{int} and \ac{ext} in the same situation. This leaves the possibility to have a checkmark at \ac{int}, at \ac{ext} or at none of them. This gives 3 × 3 = 9 logically possible options.
...
In what follows I show how Figure \ref{fig:typology} of all logically possible patterns only generates the attested patterns (except for the one in which the relative pronoun always takes the internal case).

I start with the leftmost pattern in Figure \ref{fig:typology}. This is the one in which there is no case competition, and the relative pronoun surfaces in the internal case. As I mentioned earlier, I am not aware of a language that exemplified this pattern, and future research should tell whether this option is attested or whether it should be excluded. This pattern is numbered as 1 in Table \ref{tbl:possible-headless-relatives}.
The second pattern in Figure \ref{fig:typology} is the one in which there is no case competition, and the relative pronoun surfaces in the external case. This pattern is exemplified by Old English and Modern Greek, and it is numbered as 5 in Table \ref{tbl:possible-headless-relatives}.
The third pattern in Figure \ref{fig:typology} is the one in which there is case competition, and the relative pronoun is neither allowed to surface in the internal case nor in the external case when either of them wins the case competition. This pattern is exemplified by Polish, and it is numbered as 9 in Table \ref{tbl:possible-headless-relatives}.
The fourth pattern in Figure \ref{fig:typology} is the one in which there is case competition, and the relative pronoun is only allowed to surface in the internal case when it wins the case competition. This pattern is exemplified by Modern German, and it is numbered as 3 in Table \ref{tbl:possible-headless-relatives}.
The fifth and last pattern in Figure \ref{fig:typology} is the one in which there is case competition, and the relative pronoun is allowed to surface in the internal case when either of them wins the case competition. This pattern is exemplified by Old High German, Gothic and Classical Greek, and it is numbered as 2 in Table \ref{tbl:possible-headless-relatives}.

This leaves four patterns that are logically possible but not attested in languages: 4, 6, 7 and 8. These patterns cannot be generated by Figure \ref{fig:typology}.
In the pattern numbered as 4, the relative pronoun surfaces in the external case when the internal case is the most complex, and the relative pronoun surfaces in the internal case when the external case is the most complex. In other words, the relative pronoun  appears in the losing case in the case competition.
In the pattern numbered as 6, the relative pronoun surfaces in the external case when the internal case is the most complex, and there is no grammatical option when the external case is the most complex. Here again, the relative pronoun appears in the losing case in the case competition.
In the pattern numbered as 7, there is no grammatical option when the external case is the most complex, and the relative pronoun surfaces in the internal case when the external case is the most complex. Here again, the relative pronoun appears in the losing case in the case competition.
In the pattern numbered as 8, the relative pronoun is only allowed to surface in the external case when it wins the case competition. I discuss in Chapter \ref{ch:decomposition} why this is impossible for a headless relatives.
