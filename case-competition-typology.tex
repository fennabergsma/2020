...
\tit{queman} `to come' takes nominative subjects.
\tit{senten} `to send' takes nominative subjects.
...
\tit{gihoren} `to listen to' takes accusative objects.
\tit{quedan} `to speak' takes accusative objects.

\exg. gihortut ir \tbf{thiu} \tbf{ih} \tbf{íu} \tbf{quad}\\
 listen.\ac{pst}.2\ac{pl}\scsub{acc} 2\ac{pl}.\ac{nom} \ac{rel}.\ac{pl}.\ac{m}.\ac{nom} 1\ac{sg}.\ac{nom} 2\ac{pl}.\ac{dat} speak.\ac{pst}.1\ac{sg}\scsub{acc}\\
 `you listened to those things, that I said to you' \flushfill{Old High German, \ac{tatian} 165:6}\label{ex:ohg-acc-acc}

I could find an example from the `Heliand', an Old Saxon text written around the same time as the other works I give examples from. Old Saxon is linguistically speaking the closest relative of Old High German.

\tit{seggian} `to say' takes dative indirect objects.
\tit{willian} `to wish' takes dative objects.

\exg. sagda \tbf{them} \tbf{siu} \tbf{uuelda}\\
 say.\ac{pst}.3\ac{sg}\scsub{dat} \ac{rel}.\ac{pl}.\ac{m}.\ac{nom} 3\ac{sg}.\ac{f}.\ac{nom} wish.\ac{pst}.3\ac{sg}\scsub{dat}\\
...
\tit{besuchen} `to visit' takes nominative subjects.
\tit{mögen} `to like' takes nominative subjects.
...
\tit{einladen} `to invite' takes accusative objects.
\tit{mögen} `to like' takes accusative objects.
...
\tit{vertrauen} `to trust' takes dative objects.
\tit{gefällen} `to please' takes dative subjects.
...
\tit{lubić} takes accusative objects.
...
\tit{pomagać} `to help' takes dative objects.
\tit{ufać} `to trust' takes dative objects.

\exg. Jan pomaga \tbf{komu} \tbf{-kolkwiek} ufa.\\
 ...
 \begin{table}[H]
   ...
   \begin{tabular}{c|c|c}
     ...
     \textsubscript{\ac{int}} \textsuperscript{\ac{ext}}
      ...
      \bottomrule
   \end{tabular}
     \label{tbl:case-competition-polish-matching}
 \end{table}
